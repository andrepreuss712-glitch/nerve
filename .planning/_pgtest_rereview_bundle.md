...
===== FILE: tests/test_rls_isolation.py =====
...
===== FILE: tests/test_anonymizer_worker.py =====
...
===== FILE: app.py seeders (lines 1125,1145p;1196,1245p;1310,1320p;2120,2135p) =====


===== FILE: models.py public/training region (lines 1,30p;249,270p;505,600p) =====
...
===== FILE: alembic/versions/0008_training_schema_foundation.py =====
...
<task type="auto" tdd="false">
  ...
  <action>
    1. **Helfer in conftest.py** (nach den Imports / bei den Seed-Helfern aus Task 1): implementiere
       ...
       - Best-effort: `try: <deletes>; commit except Exception as e: rollback; <LAUTE Warnung>`. **#5:** im
       ...
       - Docstring: nenne das Vorbild (test_rls_isolation.py:101-116), die reverse-FK-Ordnung, die Tenant-GUC-
         Pflicht für crm.*, die LAUTE-Warnung-bei-Cleanup-Fehler-Semantik (#5), und die Konvention (s.u.).
  ...
  <done>
    ...
    die übergebenen test-eigenen IDs, fasst Baseline-Rows nicht an) + den Konventions-Kommentar. **#5:** im
...
<task type="auto" tdd="false">
  ...
  <behavior>
    ...
    - SESSION-START-SNAPSHOT (PUBLIC.*-only): eine session-scoped Fixture (die NACH den app-import-Seeds + dem
      ...
      ein, gelesen ueber die EIGENE session-scoped Read-Engine (#2). Relevante public Tabellen (die committed-data-
    ...
    - PER-TEST-PRUEFUNG (autouse, POST-yield NACH dem Test-eigenen cleanup_rows): eine autouse function-scoped
      Fixture liest pro relevanter PUBLIC Tabelle das AKTUELLE PK-Set ueber die EIGENE session-scoped Read-Engine
      (#2, nerve_app, kein RLS auf public) und asserted == Baseline-PK-Set. Extra-PKs (Leak: Test hat committed +
      nicht aufgeraeumt) ODER fehlende Baseline-PKs (Test hat Baseline-Row geloescht) → FAIL-CLOSED, mit klarer
      Message: nodeid (welcher Test) + Tabelle + die konkreten extra/missing PKs. crm.*/training.* werden hier
      NICHT geprueft (POST-SUITE in Plan 02).
  ...
  <action>
    1. **EIGENE session-scoped Read-Engine (#2) + Snapshot-Fixture** (session-scoped, depends on dem Base-Seed
       ...
       und dispose sie am Session-ENDE (yield-Fixture: `yield engine; engine.dispose()`). Lies pro relevanter
       PUBLIC Tabelle das PK-Set ueber diese Read-Engine (nerve_app, kein RLS auf public). Speichere
       `BASELINE = {tabelle: frozenset(pks)}` in einem session-globalen Objekt. KEINE crm.*/training.*-Arme,
    ...
    2. **Waechter-Fixture** (`@pytest.fixture(autouse=True)`, function-scoped): NACH `yield` (also nach dem Test
       ...
       pro PUBLIC Tabelle das aktuelle PK-Set ueber die EIGENE session-scoped Read-Engine (#2, nerve_app) und
       asserted `current_pks == BASELINE[tabelle]`. Bei Abweichung:
       `pytest.fail(f"[BASELINE-GUARD] {request.node.nodeid}: Tabelle {t} drifted — "
       f"leaked={current-baseline}, missing={baseline-current}")` (fail-closed). KEINE crm.*/training.*-Arme —
  ...
  <done>
    ...
    Base-Seed (Task 4) + app-import-Seeds das PK-Set jeder relevanten PUBLIC Daten-Tabelle ueber die eigene
    Read-Engine ein — KEINE crm.*/training.*-Arme, KEIN superuser-Conn, KEIN NERVE_BASELINE_GUARD_DSN) + die autouse
    function-scoped `_baseline_cleanup_guard` (asserted nach JEDEM Test — NACH dem Test-eigenen cleanup_rows —
    current_pks == baseline pro PUBLIC Tabelle ueber die eigene Read-Engine; Drift → fail-closed mit nodeid + Tabelle
    + leaked/missing PKs). Weil der Waechter-Teardown ZULETZT laeuft (nach dem MODUL-Engine-dispose von db_session/
    ...
    (leakender public-Test → Waechter rot; sauberer → gruen) als echte Assertion (keine Source-Presence). Die
...
| `cleanup_rows({tabelle: ids}, tenant)` | committender Test ruft im POST-yield-Teardown; bei Fehler LAUTE [PGTEST-CLEANUP]-Warnung (#5) | reverse-FK DELETE der test-eigenen Rows (crm.* unter Tenant-GUC, public.* ohne) | ✓ Vorbild test_rls_isolation.py:101-116; loescht NUR uebergebene IDs, nie Baseline (T-PGTEST-24); Cleanup-Fehler laut (#5) |
...
| `_baseline_snapshot` PK-Sets | Session-Start nach Base-Seed: NUR public.* ueber die eigene Read-Engine (#2) | in-memory `{tabelle: frozenset(pks)}` (Vergleichsbasis, public-only) | ✓ Baseline-Contract (ENUMERATION); crm.*/training.* NICHT im Snapshot (POST-SUITE Plan 02) |
| `_baseline_cleanup_guard` Vergleich | autouse POST-yield NACH Test-cleanup: current_pks vs baseline (public-only) ueber die eigene Read-Engine (#2) | DB-State-Read (public.* eigene Read-Engine, NICHT MODUL-SessionLocal) | ✓ fail-closed bei Drift; Ordering: Pruefung nach Test-Teardown (Docstring + frueh-angefordert); eigene Read-Engine (#2) verhindert UnboundExecutionError |
...
- Extension 1 (T-PGTEST-24 + #5): `grep "def cleanup_rows(" tests/conftest.py` + `grep "PGTEST-CLEANUP" tests/conftest.py` + `grep cleanup_rows CLAUDE.md` → present; Helfer modelliert auf test_rls_isolation.py:101-116; bei Cleanup-Fehler LAUTE Warnung (#5); CLAUDE.md traegt die Konventionsregel.
- Extension 2 (T-PGTEST-24/25/31): `grep "_baseline_cleanup_guard" tests/conftest.py` present (PUBLIC.*-only); `grep "_baseline_guard_engine" tests/conftest.py` present (eigene session-scoped Read-Engine, #2); `grep "NERVE_BASELINE_GUARD_DSN" tests/conftest.py` LEER; `grep -c "UnboundExecutionError" /tmp/pgtest_deploy1d.log` == 0 (#2); im Gate laeuft der Waechter autouse nach jedem Test (public.*) ohne Crash; tests/test_baseline_guard.py PASSED (Mechanismus-Beweis: leakender public-Test → rot); kein faelschlicher Drift-FAIL fuer korrekt aufraeumende Tests; jeder committende Test (Plan 03/04) == public-Baseline nach Teardown.
