...
Next: Welle 1 ✅ DEPLOYED. STOPP-PUNKT (D-02). Welle 2 (Plan 02 — crm-Schema + 4 Tabellen + RLS-Kit + Session-UUID-Enrichment + Pre-Call-Briefing + CSV) ist der naechste Schritt, ABER: (a) startet mit STEP-0-HARD-GATE der den Welle-1-Prod-Zustand als postgres re-verifiziert (tenant_orgs-Shape, trg_mk_tenant_org da, mk_tenant_org=INVOKER, count==organisations, 0 NULL calls.tenant_id, head=0011), (b) Meeting-Modal-Frontend braucht VORHER `/gsd-ui-phase 08.23.2.G-MEET` (DSGVO Art.25 opt-in — Datenschicht/RLS/Briefing/CSV laeuft ohne). Kommando fuer Welle 2: `/clear` → `/gsd-execute-phase 08.23.2.G-MEET --wave 2` (manual-direct-prod, Migration 0012 als postgres, NICHT auto-advancen). Welle 3 danach = Migration 0013 (training.preference_pairs EXTEND + worker-policies). OFFENE FOLLOW-UPS aus D.UX (klein, kein Blocker): (1) test_postcall_split.py Fixture in-memory; (2) Plan-Verify node --test; (3) /api/health git_head stale; (4) bare-document in Live-Mode-Code auf pipEl; (5) Hold-Knopf-UX; (6) Backlog 999.2 Per-Call-Lernkarten.
