...
status: Phase 08.23.2.D.UX.4 DEPLOYED auf Production (Plan 01+02+03 code-complete, manual-direct-prod tar-Deploy 2026-05-31, Service aktiv+healthy, neue Routes live). WARTET auf Andre Live-UAT. Test-Caveats: test_postcall_split sqlite-Fixture-Bug (postgres-only CREATE SCHEMA in alembic 0008, pre-existing), node fehlt auf Prod (modal-test nur lokal gruen 6/6).
...
Phase: 08.23.2.D.UX.4 — DEPLOYED auf Production, wartet auf Andre Live-UAT (3/3 Plans code-complete, alle gepusht origin/main bis f635ad9)
Next: Andre Live-UAT auf https://getnerve.app (Call beenden -> Hold-to-end 600ms -> Ladebalken 1 -> Outcome-Screen 3 States -> Confirm -> Score+Analytics SICHTBAR im PiP). Bei gruen: Plans complete markieren + Phase abschliessen. Bei Bug: gsd-debug. FOLLOW-UPS: (1) test_postcall_split.py Fixture auf in-memory-pattern statt alembic-CREATE-SCHEMA (sqlite-inkompatibel) fixen; (2) Plan-Verify node --test braucht node auf Prod ODER lokal-only deklarieren; (3) /api/health git_head stale (a2d7d3c) weil manual-direct-prod kein .deploy_meta schreibt. Backlog: 999.2 Per-Call-Lernkarten-Anzeige (Phase-4.11-Placeholder), ART17-PURGE (🔴), LOGIN-Haertung (🟡).
Last activity: 2026-05-31 -- Phase 08.23.2.D.UX.4 EXECUTED + DEPLOYED (manual-direct-prod). Task 0 HIGH-1-Gate PASSED (Prod-Realdaten non-default). Code: 01 Score-Split, 02 Postcall-Split (outcome+cards Endpoints live HTTP 400, monolith-Shim erhalten), 03 Frontend-Reorder (pipEl-BLOCKER-Fix, hold-to-end, Option-3: keine Karten/Ladebalken-2 im PiP). Verify-Strategie auf manual-direct-prod umgestellt (tar-over-ssh, kein git pull — Prod ist tar-deployed mit .git excluded, deploy.sh haengt am kaputten Staging-Gate). Pre-Deploy-Audit: Prod-Source == pre-phase main (kein Hotfix geklobbert; nur tote app.js/app.html aus 08.11-Delete). 13/13 Backend-Logic-Tests gruen, Service aktiv+healthy. Commits 8505a49/fde69fe/8ca6b52 (01), 5efa559/c1ca118 (02), cbd0d35/97e46e8/4328e97 (03). Multi-Segment-ID — Pfade hardcoded, STATE hand-editiert.
