...
status: Phase 08.23.2.G/MEET 🟡 IN EXECUTION — Welle 1/3 ✅ LIVE AUF PROD 2026-06-01 (migration head=0011, git_head ed8a137). tenant_orgs UUID-Achse + Dual-Write-Trigger (SECURITY INVOKER, prosecdef=f verifiziert) + calls.tenant_id-Backfill (0 Orphans, hard-RAISE-Guard bestanden) + TenantOrg ORM. tenant_orgs 1:1 seeded (2==2). 6 Tests gruen server-side. STOPP nach Welle 1 (D-02) — NICHT auto-advanced (Prod-Migration bestaetigungspflichtig). Naechster Schritt: Welle 2 (Plan 02, crm-Schema + RLS) startet mit STEP-0-HARD-GATE der den Welle-1-Prod-Zustand re-verifiziert; vorher /gsd-ui-phase fuer Meeting-Modal-UX (Datenschicht laeuft ohne). Welle 3 = training.preference_pairs (EXTEND). Bewiesene Prod-Konstanten in 01-SUMMARY (PG=16.14, nerve_app bypassrls=f/rolconfig leer, training existiert/crm fehlt, kein Pooler) — Welle 2/3 erben sie als FAKTEN. Multi-Segment-ID — Pfade hardcoded, gsd-tools umgangen, STATE hand-editiert.
...
last_updated: "2026-06-01T14:40:05.000Z"
...
Phase: 08.23.2.G/MEET — 🟡 IN EXECUTION, Welle 1/3 ✅ LIVE AUF PROD 2026-06-01 (head=0011, git_head ed8a137). Welle 2 + 3 ausstehend (Stopp nach jeder Welle, D-02). Verzeichnis hartkodiert: `.planning/phases/08.23.2.G-MEET-foundation-conversational-memory-crm-multi-tenancy-training/`.
Next: Welle 2 (Plan 02) CODE KOMPLETT + COMMITTED auf main (head 8c38bdc), STATUS = AWAITING-DEPLOY (NICHT complete). STEP-0-HARD-GATE bestanden (Welle-1-Prod-Zustand als postgres re-verifiziert: tenant_orgs-Shape ok, trg_mk_tenant_org da, mk_tenant_org=INVOKER, 2==2, 0 NULL calls.tenant_id, head=0011, nerve_app rolbypassrls=f, crm absent). Gebaut: Migration 0012 (crm-Schema + 4 RLS-Tabellen, FORCE RLS, USING+WITH CHECK current_setting missing_ok, tenant+FK+GIN-Indizes, KEIN ALTER ROLE), 4 crm-ORM-Modelle ({'schema':'crm'}), contextvar + Session after_begin transaction-local set_config(...,true), Login-tenant_id-Enrichment, account_memory Pre-Call-Briefing-Merge + register_briefing_pii-Wiring, /crm/export/accounts|contacts.csv (RLS-scoped), RLS-Isolations-Test (real-PG, kein SQLite-Fallback) + Briefing-Test. KEINE Prod-Migration/Deploy/pytest ausgefuehrt (HART: kein local pytest; RLS-Test braucht real-PG nur auf prod). André faehrt den Prod-Deploy selbst: siehe 08.23.2.G-MEET-02-SUMMARY.md Sektion (c) — exakte manual-direct-prod-Befehle (alembic upgrade head als postgres → 0012; pg_class/pg_policy-Verify; deploy.sh production; NERVE_APP_TEST_DSN-gesetzt pytest tests/test_rls_isolation.py + test_account_memory_briefing.py server-side). Plan/Phase erst NACH gruenem real-PG-Test als complete markieren. Out-of-request crm-Reader-Audit: KEIN Reader gefunden (N/A clean). Meeting-Modal-Frontend braucht weiterhin `/gsd-ui-phase 08.23.2.G-MEET` (deferred, Datenschicht laeuft ohne). Welle 3 danach = Migration 0013 (training.preference_pairs EXTEND + worker-policies). OFFENE FOLLOW-UPS aus D.UX (klein, kein Blocker): (1) test_postcall_split.py Fixture in-memory; (2) Plan-Verify node --test; (3) /api/health git_head stale; (4) bare-document in Live-Mode-Code auf pipEl; (5) Hold-Knopf-UX; (6) Backlog 999.2 Per-Call-Lernkarten.
