...
stopped_at: Phase 08.23.2.STT (Deepgram-Qualität: nova-3 + keyterm + Cold-Call-Sprecher-Label-Fix) 🟡 — ✅ EXECUTED 2026-06-05 (beide Plans, 2 Waves sequenziell). Cross-AI (Gemini) → `--reviews`-Replan (HIGH try/except keyterm-SDK-Fallback + MEDIUM A1-Verify-Schärfung + MEDIUM DB-Smell-Note + LOW deferred), plan-checker re-PASSED. Code-Commits 3705664 (build_keyterms+Grundliste) / 4e86798 (nova-3+keyterm+Fallback) / dce96d7 (Reorder Mini-Load) / 691b421 (mode Closure-Capture) / 01ce28f (cold_call Hard-Label+sp_name) + SUMMARYs 550557b/14d335a. Code-Level-Must-Haves statisch verifiziert (kein nova-2 mehr, options_kwargs.pop-Fallback, _make_on_message(sid,mode), cold_call→elif-roles_confirmed, persist+diarize intakt). **⏳ Production-Verifikation OFFEN (HART: nur Prod testbar) → André `bash deploy.sh production` + 1x cold_call + 1x meeting Test-Call, Claudian `inspect.sh logs`/`sample`. Items in `08.23.2.STT-HUMAN-UAT.md` (9 pending).** NICHT auto-advanced (human_needed). gsd-sdk/gsd-tools/gsd-code-review/gsd-verifier umgangen (Multi-Segment-Gotcha + query-Interface unavailable), Pfade hardcoded, STATE/ROADMAP hand-editiert, lokale Test-Gates per HART übersprungen. --- PARALLEL OFFEN: Phase 08.23.2.D.UX.2 CODE-COMPLETE + Toggle-Bug gefixt (debug pip-toggle-invisible, head bd51cca), NICHT deployed → André `bash deploy.sh production` + Live-UAT ausstehend (PiP Toggle jetzt teal sichtbar, Dashboard Reiter+Suche+Copy, Post-Call inline). SYSTEM-Speaker-FLAG bei UAT bestätigen.
last_updated: "2026-06-05T16:30:00.000Z"
duux3_complete: "Phase 08.23.2.D.UX.3 (Anonymisierungs-Tuning) ✅ COMPLETE + UAT PASS 2026-06-05, live auf Production. 4/4 Tasks. Commits ef8ec32/9727fe9/332aa25 (Core) + 18a95a1 (Folge-Fix-Pass 1). RESEARCH korrigierte ROADMAP (Wortteil-Bug in anonymize_output, nicht _apply_ner; Pronomen-Whitelist=Wurzel). Gemini --reviews eingearbeitet (THRESHOLD 0.55+ENV, _is_whitelisted-Pflicht-Helper, 5-10-Call-Korpus-Gate). Folge-Fix nach 1. UAT (Prod-Log Call 15:04): _dedup_overlapping_spans (Doppel-Klammer [PERSON_E]SON_D] + ORG-Teil-Leak Brennecke = überlappende Union-Voting-Spans) + _is_whitelisted typ-unabhängig+Mehrwort-Check (generische Über-Schwärzung wir Vertriebler/Vertriebsteams/Einkauf). Re-Test approved: 3 Abweichungen weg, DSGVO-Gate hält (alle echten Namen geschwärzt @0.55). Blocker für Phase 08.23.2.E gelöst. NICHT auto-advanced. Multi-Segment-Gotcha: Pfade hardcoded, gsd-sdk/gsd-code-review/gsd-verifier umgangen, STATE/ROADMAP hand-editiert."
duux3_planned: "Phase 08.23.2.D.UX.3 (Anonymisierungs-Tuning, services/anonymization.py + 1 Wiring-Zeile deepgram_service.py) GEPLANT + CROSS-AI (Gemini) + --reviews-Replan 2026-06-05. 1 Plan/4 Tasks (3 auto + Task-4 human-verify-checkpoint), plan-checker 2x PASSED. RESEARCH korrigierte ROADMAP: Wortteil-Bug sitzt in anonymize_output (naked text.replace), NICHT _apply_ner; Pronomen-Whitelist = Wurzel-Fix. Gemini-Findings eingearbeitet: GLINER_THRESHOLD Default 0.55 (nicht 0.6) + ENV-Override, Task-4 gegen 5-10 historische Prod-Calls (kein echter Name @0.55 durchgelassen, DSGVO Art.32), _is_whitelisted-Helper verpflichtend, defensiver Dict-Zugriff, Firmenname-Exact-Match als Known-Limitation. Verzeichnis hardcoded .planning/phases/08.23.2.D.UX.3-anonymisierungs-tuning/. NÄCHSTER SCHRITT: /gsd-execute-phase 08.23.2.D.UX.3 (Cross-AI erledigt, Findings drin). Multi-Segment-Gotcha: Pfade hardcoded, gsd-sdk/gsd-tools umgangen, STATE/ROADMAP hand-editiert. Verify=Production-only."
taxo1_planned: "Phase 08.23.2.TAXO1 (Verstehen — Fundament + Erkennung) 🔴 GEPLANT 2026-06-10 — 7 Plans / 7 serielle De-Risk-Wellen (SCHILD-Stil), plan-checker PASSED 1. Iteration (0 Blocker, 1 WARNING gefixt: Plan 06 depends_on→[03,05] für lineare Alembic-Kette 0016→0017→0018). Verzeichnis hardcoded `.planning/phases/08.23.2.TAXO1-verstehen-fundament-erkennung/`. RESEARCH.md (540 Z., Verbindungs-Karte §9 grep+inspect-belegt) korrigierte Blueprint-Anker gegen Live-Code: ewb_ratings hat 5 Reader in admin_ewb.py (NICHT 0); K3-Wurzel exakt ki_logik.py:178 (`raw_phase > current_phase` int>str); Migration-HEAD 0015→neue 0016; org_id-K8-Wurzel = Live-Loop loggt user_id=None hardcoded (claude:489). Wellen: 1 intent-event-Tabelle+Schild (schema-addition, REQ 1/2, vollständiger Klebstoff-Contract §3: 9 indiz. Spalten+payload_jsonb+4+3 Pflichtfelder, handling_score_numeric NULL/kein Scoring, taxonomy_version Pflicht, INTENT_TAXONOMY_V1 als EINE Quelle D-01); 2 Drei-Bahnen-Gerüst (Queue+1 Daemon-Consumer+Adapter+Graceful-Flush, leer, Bau-Regel 1/2, schema-addition REQ 3); 3 §0.1-Putzliste-Fold+K3+org_id (riskant, REQ 8/4, Test-Anruf, 12 Verletzungen je 4-Schicht+Race-Audit, Globale GELÖSCHT); 4 Live-Cutover Fast+Medium→intent_event+§1-Taxonomie+K4-Threshold (riskant, REQ 4/5, Test-Anruf; K4 NICHT als Pipeline-Bug überbaut — Single-Speaker-Funnel korrekt, nur Threshold+abstain); 5 Zombie-Rename ewb_ratings+objection_events-Dual-Write-Shim `# SUNSET: TAXO2` (riskant, REQ 7, Test-Anruf+man. SCHILD-Guard); 6 EWB v2-modular als EINE Variante (riskant, REQ 9, Test-Anruf+ANKER/REFRAME-Check); 7 ModeStrategy-Registry+2 Klassen+3 Steckplätze (riskant, REQ 6, Test-Anruf). Alle 9 SPEC-REQs abgedeckt. Production-only Verify (Migration als postgres+OWNER nerve_app, man. Guard, kein lokales pytest als Acceptance). **🔴 → Cross-AI PFLICHT vor Execute (André-Direktive: TAXO1/2/3 alle bis kurz vor Execute, dann Ineinandergreifen prüfen).** **CROSS-AI (Gemini) + `--reviews`-REPLAN 2026-06-10:** REVIEWS.md (1 HIGH + 1 MEDIUM + 2 LOW) eingearbeitet in 4 Plans (02/03/04/05), Plans 01/06/07 unverändert (Gemini-Interlock sauber). #1 HIGH Slow-Lane-Durability: KEIN file-spill — als Design-Prinzip aufgelöst (Plan 02 §0: intent_event ist Single Truth, durabel von emit_intent_event/Plan 04 ZUERST geschrieben; Queue hält nur event_id-Verweis, Arbeits-Liste re-derivierbar `WHERE handling_score_numeric IS NULL`; _persist_event_ref = echter No-Op in TAXO1 → Plan02↔04-Inkonsistenz an der Wurzel gelöst, emit_intent_event ist einzige Schreibstelle). #2 MEDIUM: SIGTERM-Handler zusätzlich zu atexit (Plan 02 Task 2). #3 LOW: mirror_intent_events_to_objection_events idempotent (keyed conversation_log_id, DELETE-then-INSERT, Plan 05) → doppeltes /api/beenden zählt nicht doppelt. #4 LOW: K3-int/str + K4-Threshold-Funnel pure-logic Regression-Tests (Plan 03 Task 5 test_k3_phase_coercion.py + Plan 04 Task 5 test_k4_threshold_funnel.py via reiner should_abstain()-Helper in config.py). plan-checker re-PASSED (0 Blocker, 2 Warnings gefixt: test_intent_event_writer.py in Plan04 files_modified, 'Task 6'→'Checkpoint'-Labels). Multi-Segment-Gotcha: Pfade hardcoded, gsd-tools/gsd-code-review/gsd-verifier umgangen, STATE/ROADMAP hand-editiert. NICHT auto-advanced."
last_activity: "2026-06-01 -- Phase 08.23.2.G/MEET Welle 1 EXECUTED + manual-direct-prod deployed (André "I drive it now"). Migration 0011 als postgres → head=0011. tenant_orgs (id uuid PK gen_random_uuid, legacy_org_id int UNIQUE FK, owner nerve_app) 1:1 seeded (2==2), trg_mk_tenant_org AFTER INSERT → mk_tenant_org() SECURITY INVOKER (prosecdef=f, proconfig leer — Gemini Q1 T-G1-01), calls.tenant_id-Backfill mit post-backfill hard-RAISE-Guard (Gemini Q3, 0 Orphans bestaetigt). TenantOrg ORM (DDL-parity). 6 Tests gruen server-side. Commits 8e6e11e (RED), 4461987 (migration), ed8a137 (GREEN ORM). /api/health git_head=ed8a137 status ok. gsd-code-review GESKIPPT (Multi-Segment-ID-Gotcha → review wuerde leeren Falsch-Dir lesen; manueller Review via Cross-AI-eingearbeitete Plan-Findings stattdessen). Vorher: Phase 08.23.2.D.UX.4 COMPLETE 2026-05-31. Task 0 HIGH-1-Gate PASSED. Live-UAT-Bug gefixt: leeres PiP nach Confirm — Root-Cause _showLadebalken1 setzte #nlp-section-postcall display:none, aber genau diese Section ist der sichtbare Container fuer Ladebalken 1 + Outcome-Screen (#pip-postcall existiert nicht → Host-Fallback); nichts machte sie vor Confirm sichtbar → Outcome-Screen unsichtbar → Confirm nie geklickt → leeres PiP. Fix: Section sichtbar (display:flex), nur Score-Karten-Kinder bis Confirm versteckt. (NICHT bare-document — _renderOutcomeUx war bereits pipEl-korrekt.) Logging-first diagnostiziert (Andre live), kein Blind-Fix. Debug-Session resolved: .planning/debug/resolved/pip-score-empty-after-confirm.md. Commits 82b25ec (fix), a718a4e (logs gestrippt). Multi-Segment-ID — Pfade hardcoded, STATE hand-editiert."
progress:
  total_phases: 96
  completed_phases: 55
  total_plans: 237
...
Last session: 2026-06-03T13:30:56.104Z
Stopped at: Phase 08.23.2.D.UX.2 UI-SPEC approved
Resume file: .planning/phases/08.23.2.D.UX.2-transcript-reiter-ui-pip-auswertung-dashboard/08.23.2.D.UX.2-UI-SPEC.md
