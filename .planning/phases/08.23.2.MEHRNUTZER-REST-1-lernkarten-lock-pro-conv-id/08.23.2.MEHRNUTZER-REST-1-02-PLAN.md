...
must_haves:
  truths:
    ...
    - "Der Waechter erkennt `with _analysis_lock_for(conv_id):` als bewachten Block (er wird nach dem Fix nicht blind) und meldet ihn zugleich NICHT als Verstoss (das gruene Tor bleibt erreichbar)."
...
Output: ein neuer Abschnitt **am Dateiende**. Die **sechs** bestehenden Tests werden **nicht**
...
diesen Abschnitt anhaengen. **Die sechs bestehenden Tests und `_WHITELIST` bleiben unberuehrt.**

⚠ **Die Datei hat SECHS Testfunktionen, nicht vier** (`grep -c "def test_"` → 6):
`test_killed_module_globals_absent:209`, `test_killed_state_keys_absent:224`,
`test_no_new_live_global_state_writes:265`, `test_deprecated_globals_not_newly_written:320`,
`test_pending_migration_wave_tags_are_valid:365`, `test_pending_migration_is_empty:377`.
Alle Zahlen-Anker in diesem Plan rechnen mit 6. **Eine Zahl, die nicht stimmt, verleitet den
Executor zum billigsten Weg: Tests loeschen** — genau der Abrieb, den die Test-Netz-Ratsche
verhindern soll.
...
Sortierung, keine Aenderung an `_WHITELIST`, `_PENDING_MIGRATION` oder den **sechs** bestehenden Tests.
  </action>
  ...
  <acceptance_criteria>
    ...
    - `grep -c "def _ist_gemeinsamer_riegel_ausdruck" tests/test_no_live_global_state.py` → 1
    - `grep -c "def _ist_per_schluessel_riegel_ausdruck" tests/test_no_live_global_state.py` → 1
    - Alle **sechs** bestehenden Tests unangetastet:
      `grep -c "def test_no_new_live_global_state_writes\|def test_pending_migration_is_empty\|def test_killed_module_globals_absent\|def test_killed_state_keys_absent\|def test_pending_migration_wave_tags_are_valid\|def test_deprecated_globals_not_newly_written" tests/test_no_live_global_state.py` → 6
...
<task type="auto" tdd="true">
  ...
  <behavior>
    ...
    - Selbst-Test 3 (**L3-Beweis**, beide Richtungen): `with _analysis_lock_for(conv_id):` wird
      als bewachter Block **gezaehlt** (== 1), aber **NICHT gemeldet** (per-Schluessel-Riegel ist
      kein Verstoss) — **und** ein danebenstehendes `with _globaler_lock:` mit `messages.create`
      **muss** weiterhin gemeldet werden.
  </behavior>
...
```
  ...
  <verify>
    <automated>python -c "import ast; s=open('tests/test_no_live_global_state.py',encoding='utf-8').read(); ast.parse(s); assert s.count('def test_')==11"</automated>
    <!-- 6 bestehende + 5 neue. Scharfe Abnahme: Plan 03 (Tor auf dem Server). -->
  </verify>

  <acceptance_criteria>
    - `grep -c "def test_" tests/test_no_live_global_state.py` → 11 (**sechs** bestehende + 5 neue)
    ...
    - `grep -cF "_globaler_lock" tests/test_no_live_global_state.py` → mindestens 1
      (die Gegenprobe III MUSS da sein — sonst prueft der L3-Test nur eine Richtung)
    - `grep -cF "_hole_lock()" tests/test_no_live_global_state.py` → mindestens 1
  ...
  <done>
    ...
    (`test_riegel_erkennung_erfasst_context_manager_aufruf`) existiert und assertiert **alle vier**
    Richtungen: Fabrik-Block gezaehlt (== 1), Fabrik-Block **nicht** gemeldet, gemeinsamer Riegel
    weiterhin gemeldet, Fabrik ohne Argument gemeldet.
  </done>
</task>
...
```
  ...
  <acceptance_criteria>
    ...
    - `grep -c "def test_" tests/test_no_live_global_state.py` → 12 (**sechs** bestehende + 6 neue)
  ...
  <done>
    ...
    (plus die STOP-REGEL), benennt die durch die Zaehl-/Melde-Trennung geoeffnete Restluecke,
...
<verification>
- `tests/test_no_live_global_state.py` parst; **12** Tests (**sechs** bestehende + 6 neue).
- Genau **ein** neuer Test traegt `rot_vor_fix`.
- `_WHITELIST` / `_PENDING_MIGRATION` / die **sechs** bestehenden Tests unveraendert.
- Der L3-Selbst-Test existiert und prueft **alle vier Richtungen** (gezaehlt / nicht gemeldet /
  gemeinsamer Riegel weiterhin gemeldet / Fabrik ohne Argument gemeldet).
...
- ⚠ Falls im ROT-Lauf **zusaetzlich** `test_riegel_sweep_erreicht_alle_bekannten_bloecke` failt,
  ...
  Begruendung** nachziehen — **nicht** den Test entfernen und **nicht** `coaching_service.py`
  antasten. Dessen Nachzug von 1 auf 3 gehoert **ausschliesslich** in Plan 04 Task 1 (4), im
  selben Commit wie der Fix; in Welle 1 muss der Eintrag auf 1 stehen bleiben, sonst waere der
  ROT-Lauf mit drei statt zwei Fehlschlaegen rot.
</verification>
...
- Variante A erfasst `with _analysis_lock_for(conv_id):` in der **Zaehlung** und schliesst ihn aus
  der **Verstoss-Menge** aus — beide Richtungen durch einen eigenen Selbst-Test belegt.
- Drei Mindest-Soll-Anker vorhanden, `services/coaching_service.py: 1` darunter (Plan 04
  zieht diesen Eintrag im Fix-Commit auf 3 hoch).
