...
must_haves:
  truths:
    ...
    - "Das Baseline-Snapshot-Fixture importiert app SELBST als allererste Aktion (from app import app) BEVOR es das {pk:xmin}-Mapping friert (#9) — erzwingt die Modul-Top-Level-Seeder (_seed_prompt_versions/_seed_ewb_v2/_seed_founder_dashboard_defaults) gegen nerve_test, sodass prompt_versions/api_rates/fixed_costs in der Baseline enthalten sind und der erste client-ziehende Test KEIN False-Red (extra-PKs ggue. leerer Baseline) ausloest; die MODUL-Engine ist via A-1 schon nerve_test, der client-Rebind unberuehrt, sys.modules-cached"
...
<objective>
<!-- Delta-Review-6 fold 2026-06-16: #9 _baseline_snapshot importiert app SELBST zuerst (erzwingt app-import-Seeder vor {pk:xmin}-Snapshot) -> kein First-Test-False-Red. -->
...
<task type="auto" tdd="false">
  ...
  <behavior>
    ...
    - WICHTIG (Extension-2-Reihenfolge, #9 Delta-Review-6): dieser Base-Seed MUSS VOR dem Baseline-Snapshot (Task 6)
      laufen UND der Snapshot importiert app SELBST zuerst (erzwingt die app-import-Seeder), DANN friert er das
      {pk:xmin}-Mapping. Volle Reihenfolge: Base-Seed commit (Org+User id=1) -> `import app` im Snapshot-Fixture
      (die Modul-Top-Level-Seeder _seed_prompt_versions/_seed_ewb_v2/_seed_founder_dashboard_defaults feuern gegen
      nerve_test) -> {pk:xmin}-Snapshot. NICHT mehr 'annehmen der Snapshot laeuft nach app-import-Seeds' -- er
      ERZWINGT sie selbst via dem fruehen `import app` (siehe Task 6, #9). Die Org id=1 + ihre Trigger-tenant_org +
      User id=1 + die app-import-Seeds gehoeren zur Baseline (kein Leak). Stelle die Fixture-Ordering sicher
  ...
  <done>
    ...
    VOR dem Baseline-Snapshot (Task 6, der app SELBST importiert -> app-import-Seeder feuern vor dem {pk:xmin}-Snapshot, #9) -- die Base-Rows gehoeren zur erlaubten Baseline. Im Gate-Lauf (Plan 02) sind
...
<task type="auto" tdd="false">
  ...
  <behavior>
    - **#9 SNAPSHOT IMPORTIERT app SELBST ZUERST (Delta-Review-6, BLOCKER — guard-init drift):** das
      `_baseline_snapshot`-Fixture fuehrt als ALLERERSTE Aktion `import app` (`from app import app`) aus, BEVOR
      die {pk:xmin}-Snapshot-Queries laufen. Grund: der EINZIGE app-Import in irgendeinem Fixture ist der
      bewusst SPAETE Import in der function-scoped `client`-Fixture (Task 1, `from app import app` NACH der
      Rebind); der Base-Seed (Task 4) importiert nur `database.db`. Ohne den fruehen Import liefe der Snapshot
      BEVOR die Modul-Top-Level-Seeder (_seed_prompt_versions/_seed_ewb_v2/_seed_founder_dashboard_defaults)
      feuern -> prompt_versions/api_rates/fixed_costs waeren im Snapshot LEER -> der erste Test, der `client`
      zieht, triggert `from app import app` -> die Seeder feuern -> der per-Test-Waechter saehe EXTRA-PKs ggue.
      der leeren Baseline -> fail-closed False-Red auf dem ALLERERSTEN Test -> Gate blockt JEDEN Deploy.
      KOMPATIBILITAET (explizit im Fixture-Kommentar): die MODUL-Engine ist zur Import-Zeit BEREITS nerve_test-PG
      (A-1 / DATABASE_URL=postgres, commit d7d8358) -> der fruehe app-Import seedet gegen die KORREKTE DB; der
      spaetere `client`-Rebind (`SessionLocal.configure(bind=engine)`) ist unberuehrt (er regelt die Test-SESSION,
      nicht wohin die Import-Zeit-Seeder schreiben). `sys.modules`-Caching macht den spaeteren client-app-Import
      idempotent (kein Re-Seed). Reihenfolge: Base-Seed commit -> `import app` (Seeder feuern) -> {pk:xmin}-Snapshot.
    ...
    - SESSION-START-SNAPSHOT (PUBLIC.*-only): eine session-scoped Fixture, die als ERSTE Aktion `import app`
      ausfuehrt (#9 — erzwingt die Modul-Top-Level-Seeder gegen nerve_test, BEVOR der Snapshot laeuft) und die NACH dem Base-Seed (Task 4) laeuft (Fixture-Dependency/Ordering explizit), friert pro RELEVANTER public Daten-Tabelle
  ...
  <action>
    1. **EIGENE session-scoped Read-Engine (#2) + Snapshot-Fixture** (session-scoped, depends on dem Base-Seed
       aus Task 4 -> laeuft danach): **#9 ZUERST `from app import app`** ausfuehren (erzwingt die Modul-Top-Level-Seeder _seed_prompt_versions/_seed_ewb_v2/_seed_founder_dashboard_defaults gegen nerve_test, sys.modules-cached -> idempotent ggue. dem spaeteren client-app-Import; die MODUL-Engine ist via A-1 schon nerve_test, der client-Rebind unberuehrt), DANN erstelle EINMAL bei Session-Start eine dedizierte
  ...
  <done>
    ...
    Base-Seed (Task 4) + app-import-Seeds das {pk: xmin}-Mapping jeder relevanten PUBLIC Daten-Tabelle ueber die eigene Read-Engine ein; **#9: das Snapshot-Fixture fuehrt als ERSTE Aktion `import app` aus**, erzwingt damit
    die Modul-Top-Level-Seeder gegen nerve_test VOR dem {pk:xmin}-Snapshot (kein First-Test-False-Red; A-1: MODUL-Engine schon nerve_test, client-Rebind unberuehrt, sys.modules-cached) — **#7: per-Row-Change-Token via `xmin::text`, NICHT nur das PK-Set**; KEINE
...
| T-PGTEST-35 | Denial (False-Red) | Das `_baseline_snapshot`-Fixture friert die {pk:xmin}-Baseline am Session-Start, importiert app aber NICHT explizit -> die Modul-Top-Level-Seeder (_seed_prompt_versions/_seed_ewb_v2/_seed_founder_dashboard_defaults) feuern erst beim ersten `client`-Pull (spaeter app-Import) -> prompt_versions/api_rates/fixed_costs sind im Snapshot LEER -> der erste Test triggert via client `from app import app` -> Seeder feuern -> der per-Test-Waechter sieht EXTRA-PKs ggue. der leeren Baseline -> fail-closed False-Red auf dem ALLERERSTEN Test -> Gate blockt JEDEN Deploy | mitigate | #9 (Delta-Review-6, Gemini-3.1-Pro): das `_baseline_snapshot`-Fixture fuehrt als ALLERERSTE Aktion `from app import app` aus, BEVOR die {pk:xmin}-Queries laufen -> die app-import-Seeder feuern vor dem Snapshot, die Baseline enthaelt prompt_versions/api_rates/fixed_costs vollstaendig. KOMPATIBILITAET: die MODUL-Engine ist via A-1 (DATABASE_URL=postgres, commit d7d8358) schon nerve_test-PG -> der fruehe Import seedet gegen die korrekte DB; der spaetere client-Rebind (`SessionLocal.configure(bind=engine)`) ist unberuehrt (regelt die Test-SESSION, nicht die Import-Zeit-Seeder); sys.modules-Caching macht den client-app-Import idempotent (kein Re-Seed). Reihenfolge: Base-Seed commit -> `import app` (Seeder) -> {pk:xmin}-Snapshot. Acceptance: der erste Gate-Test GRUEN (kein BASELINE-GUARD-drifted-FAIL); `grep -nE "import app" tests/conftest.py` zeigt den Import im Snapshot-Fixture. Cross-ref Task 4 + Task 6 (#9). |
