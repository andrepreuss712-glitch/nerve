...
<task type="auto" tdd="false">
  ...
  <read_first>
    ...
    - tests/conftest.py client (Z.54-91) — Vorbild für die Modul-Engine/SessionLocal-Umbindung via monkeypatch
  </read_first>
  <behavior>
    - db_session: liest os.environ['TEST_DATABASE_URL']; fehlt → pytest.skip (KEIN sqlite-Fallback, D-07-Geist). Bindet das MODUL-`database.db.engine`/`SessionLocal` an die nerve_test-Engine um (NICHT lokale sessionmaker), seedet EINMAL einen Test-Mandanten (Trigger-Muster), ruft set_current_tenant(TEST_TENANT_UUID) auf, yieldet eine Session aus dem MODUL-SessionLocal, rollback/close im finally.
  ...
  <action>
    ...
    1. Modul-Konstante / Helper einführen (oben in conftest.py, nach den Imports):
       ...
       und die UUID liefert. Tag den Org-Namen `[PGTEST-GENERIC] tenant` (Identifizierbarkeit +
       Analytics-Exklusion-Lineage). `TEST_TENANT_UUID` wird beim Seed gefüllt (Discretion D-05:
       feste Konstante vs. seed-erzeugt → seed-erzeugt, FK-Zwang: crm.* FKs auf public.tenant_orgs(id),
       eine erfundene UUID würde FK-Verletzung werfen — RESEARCH Q4b).
    ...
    2. `db_session` — MUSS die MODUL-Engine umbinden (NICHT eine lokale sessionmaker bauen):
       ...
       def db_session(monkeypatch):
           ...
           try:
               ...
               engine.dispose()
       ```
       WICHTIG: NICHT `Session = sessionmaker(bind=engine)` lokal bauen. `client` bindet die
       Modul-`engine`/`SessionLocal` bereits um — `db_session` MUSS dieselbe Umbindung machen und seine
       Session aus dem MODUL-`SessionLocal` ziehen, sonst feuert der after_begin-Hook nicht. (Falls
       `SessionLocal.configure(bind=...)` im Codebase nicht das etablierte Muster ist, exakt das
       `client`-Umbindungs-Muster spiegeln — Vorbild in <read_first>.)
  ...
  <verify>
    <automated>ssh -i ~/.ssh/nerve_vps root@178.104.82.166 'cd /opt/nerve/app && grep -n "sqlite:///:memory:" tests/conftest.py | grep -v "^\s*#"' ; echo "EXIT_GREP=$?"  # erwartet: kein aktiver Treffer (Req-2-Acceptance). Voll-Beleg = deploy.sh-Gate-Lauf (Plan 02) zeigt generische crm-Tests PASSED (crm-Read unter set_current_tenant liefert geseedete Zeile, nicht 0).</automated>
  </verify>
  <done>
    `grep "sqlite:///:memory:" tests/conftest.py` liefert keinen Treffer im aktiven Fixture-Code (nur ggf.
    in Kommentaren). db_session + client binden das MODUL-`database.db.SessionLocal` an die nerve_test-Engine
    um (NICHT lokale sessionmaker), lesen TEST_DATABASE_URL und rufen set_current_tenant. ACCEPTANCE: im
    Gate-Lauf (Plan 02) liefert ein crm-Read unter set_current_tenant(TEST_TENANT_UUID) die geseedete Zeile
    zurück (≥1 Zeile, NICHT 0 — Beweis dass der after_begin-Hook auf der von db_session genutzten Session feuert);
    generische crm-berührende Tests sind PASSED (nicht 0-Zeilen-rot, nicht SKIPPED).
  </done>
</task>
...
| T-PGTEST-03 | Information Disclosure | crm-Tests ohne Tenant-Kontext ODER mit frischer sessionmaker (Hook feuert nicht) → RLS fail-closed → 0 Zeilen (Test grün trotz kaputt) | mitigate | D-05 set_current_tenant(TEST_TENANT_UUID) + tenant_orgs-Seed; db_session bindet das MODUL-SessionLocal um (Warning #3), damit der after_begin-Hook auf der genutzten Session feuert; Gate verifiziert crm-Read liefert geseedete Zeile (≥1, nicht 0) |
...
| Org-Seed `[PGTEST-GENERIC] tenant` | INSERT in `organisations.name` | DB-Spalte `public.organisations.name` | ✓ |
| `set_current_tenant(uuid)` → `app.tenant_id` GUC | after_begin-Hook auf MODUL-SessionLocal | transaktions-lokaler GUC (set_config, NICHT DB-Spalte) — gelesen von crm-RLS-Policies | ✓ db.py:87; greift NUR wenn db_session das MODUL-SessionLocal nutzt (Warning #3) |
...
- Voll-Beleg erst im Plan-02-Gate-Lauf: generische crm-Tests PASSED (set_current_tenant greift, after_begin-Hook feuert auf der von db_session genutzten MODUL-Session → crm-Read ≥1 Zeile).
</verification>
...
- db_session bindet das MODUL-`database.db.SessionLocal` um (NICHT lokale sessionmaker) → after_begin-Hook feuert; crm-Read liefert ≥1 Zeile (nicht 0).
