...
<task type="auto" tdd="false">
  ...
  <action>
    ...
    1. **test_account_memory_briefing.py — `_patched_session` (Z.20-55):**
       ...
       demselben tenant_id + account_id anlegen (analog test_rls_isolation.py:82-90). Deterministischer
       Teardown: die getaggten Rows (`[PGTEST-...]`) unter dem Tenant-GUC in reverse-FK-Reihenfolge löschen
       (account_memory → accounts → tenant_orgs → organisations), wie test_rls_isolation.py:103-113.
