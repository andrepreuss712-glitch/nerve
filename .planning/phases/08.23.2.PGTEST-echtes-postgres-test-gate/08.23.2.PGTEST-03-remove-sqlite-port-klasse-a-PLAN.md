...
- nerve_app_pg_conn-Fixture (psycopg2) ODER db_session/db_from_client (SQLAlchemy gegen TEST_DATABASE_URL).
...
<task type="auto" tdd="false">
  ...
  <read_first>
    ...
    - tests/conftest.py (NACH Plan 01: db_session/db_from_client/nerve_app_pg_conn + TEST_TENANT_UUID/Seed-Helper — das PG-Vorbild)
    - tests/test_rls_isolation.py Z.33-116 (_new_tenant Trigger-Muster + Real-Commit + reverse-FK-Teardown — D-04-Muster)
  ...
  <behavior>
    ...
    - test_anonymizer_worker.py Logic-Group: die 6 Tests laufen gegen nerve_test-PG (transcript_archive echt via alembic-gebautes Schema, KEIN raw CREATE TABLE mehr). MERGE/FILTER/HASH/GATING-Logik bleibt geprüft (_fake_anonymize-Stub bleibt — kein NLP-Load).
    - Beide nutzen das Trigger-tenant_orgs-Seed + set_current_tenant (crm-FK + RLS) und committen real mit deterministischem Teardown (Wegwerf-DB, aber Intra-Lauf-Leak-Schutz).
  ...
  <action>
    ...
    1. **test_account_memory_briefing.py — `_patched_session` (Z.20-55):**
       ...
       direkt verwenden ODER eine lokale Fixture, die `create_engine(os.environ['TEST_DATABASE_URL'])` +
       `sessionmaker` baut, einen Test-Tenant via Trigger-Muster seedet, `set_current_tenant(tenant_uuid)`
       aufruft, und `precall.get_session` auf eine PG-Session monkeypatcht. SKIP wenn TEST_DATABASE_URL
       fehlt (kein sqlite-Fallback). Das Schema NICHT mit create_all bauen — es existiert (Gate, alembic).
       Die crm.account_memory-Inserts der Tests brauchen `tenant_id` = der gesetzte Test-Tenant (RLS WITH
       CHECK) und ein vorhandenes account-FK-Ziel (crm.accounts) — passe die Test-Inserts an: vor dem
       AccountMemory-Insert eine crm.accounts-Row mit demselben tenant_id + account_id anlegen (analog
       test_rls_isolation.py:82-90). Deterministischer Teardown: die getaggten Rows (`[PGTEST-...]`) unter
       dem Tenant-GUC in reverse-FK-Reihenfolge löschen (account_memory → accounts → tenant_orgs →
       organisations), wie test_rls_isolation.py:103-113.
    2. **test_anonymizer_worker.py — `mem_engine` (Z.57-96):**
       ...
       TEST_DATABASE_URL (Schema vom Gate via alembic — training.transcript_archive existiert dann echt,
  ...
  <verify>
    <automated>bash deploy.sh production 2>&1 | tee /tmp/pgtest_deploy3.log | grep -E "test_account_memory_briefing|test_anonymizer_worker|test_rls_isolation|PASSED|SKIPPED|passed|failed"; echo "EXIT=$?"  # Req-4/Req-6: Klasse-A-Tests grün gegen PG; RLS+Anon-RLS-Gruppe PASSED nicht SKIPPED; keine Collection-Errors.</automated>
  </verify>
  <done>
    ...
    grün im Gate OHNE den ATTACH-Listener. test_rls_isolation.py + test_anonymizer_worker.py RLS-Gruppe
    erscheinen im Gate-Log als PASSED (nicht SKIPPED) — Req-4. Falls ein Test an einem ECHTEN App-Bug rot
    wird (Klasse D/E, z.B. test_ft_seed) → im SUMMARY als Fund ESKALIEREN, NICHT still patchen (SPEC out-of-scope).
  </done>
</task>
...
| T-PGTEST-12 | Information Disclosure | portierte crm-Tests ohne Tenant-Kontext → RLS fail-closed 0 Zeilen ODER BYPASSRLS-Umgehung | mitigate | set_current_tenant(TEST_TENANT_UUID) + tenant_orgs-Seed (D-05); nerve_app rolbypassrls=f; keine Superuser-Rolle |
| T-PGTEST-13 | Tampering | rollback-only für die Klasse-A-crm-Writes lässt Daten nie committen → trivial-grün (False-Green) | accept/mitigate | Klasse-A sind LOGIK-Tests (merge/filter/hash), kein RLS-Cross-Tenant → Rollback-Pfad OK; die RLS-Cross-Tenant-Prüfung bleibt im D-04-Real-Commit-Pfad der RLS-Gruppe (unverändert) |
...
- Klasse-A: test_account_memory_briefing + anonymizer Logic-Group PASSED gegen nerve_test (keine Collection-Errors).
