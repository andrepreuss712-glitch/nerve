...
must_haves:
  truths:
    ...
    - "Der dump-gebaute nerve_test trägt die echten crm-RLS-Policies/FORCE/GRANTs treu (kein False-Green)"
  artifacts:
    - path: "deploy.sh"
      provides: "Postgres-Test-Gate-Block (Provision → pg_dump-Restore-Build → pytest 4-DSN → Teardown) mit Whitelist-Guard + fail-closed"
      contains: "nerve_test"
  key_links:
    - from: "deploy.sh Gate-Block"
      ...
      pattern: "pg_dump --schema-only|--table=alembic_version|upgrade head"
    - from: "deploy.sh Gate-Block"
...
fahre pytest mit 4 nerve_test-DSNs dagegen, und räume garantiert ab. Fail-closed pro Schritt, Whitelist-
Guard `nerve_test` (D-02).
...
Build-Pfad-Skelett (RESEARCH „⚑ BUILD-PATH LOCKED" — exakt so strukturiert übernehmen):
```bash
TEST_DB="nerve_test"
...
sudo -u postgres bash -c "pg_dump --schema-only nerve | psql -v ON_ERROR_STOP=1 -d $TEST_DB" || { echo FEHLER; exit 1; }
# Stamp-Row (= prod-head) übertragen, damit upgrade nur neue Revs anwendet:
sudo -u postgres bash -c "pg_dump --data-only --table=alembic_version nerve | psql -v ON_ERROR_STOP=1 -d $TEST_DB" || { echo FEHLER; exit 1; }
...
<task type="auto" tdd="false">
  <name>Task 1: SQLite-Test-Stufe durch Postgres-Gate-Block ersetzen (Provision → pg_dump-Restore-Build → pytest 4-DSN → Teardown)</name>
  <read_first>
  ...
  <behavior>
    ...
    - Schema-Build als postgres: pg_dump --schema-only nerve → psql nerve_test; dann pg_dump --data-only --table=alembic_version nerve → psql nerve_test; dann alembic upgrade head. Jeder Schritt eigener Exit-Check + Klartext-Grund (D-07). KEIN create_postgres_schema.py, KEIN stamp 0001 (würde bei 0002 kollidieren).
  ...
  <action>
    ...
    lokale `$` escaped wie heute `\$PYTEST_EXIT` — beachte: TEST_DB/ANON_PW sind SERVER-seitige Vars, also
    `\$` escapen wo sie erst auf dem Server expandieren sollen):
    ...
    5. **Schema-Dump** (Schema+RLS+FORCE+GRANTs+FK+CHECK+Comments vom Prod-nerve, read-only auf nerve):
       `sudo -u postgres bash -c "pg_dump --schema-only nerve | psql -v ON_ERROR_STOP=1 -d \$TEST_DB" || { echo "[deploy] FEHLER: pg_dump --schema-only nerve → nerve_test fehlgeschlagen"; exit 1; }`
       WICHTIG: KEIN `--no-privileges`, KEIN `--no-owner` — die GRANTs/Owner tragen die RLS-Treue (False-Green-Schutz).
    6. **Stamp-Row-Dump** (carry die alembic_version-Row = prod-head, damit upgrade nur neue Revs anwendet):
       `sudo -u postgres bash -c "pg_dump --data-only --table=alembic_version nerve | psql -v ON_ERROR_STOP=1 -d \$TEST_DB" || { echo "[deploy] FEHLER: alembic_version-Stamp-Dump → nerve_test fehlgeschlagen"; exit 1; }`
  ...
  <verify>
    <automated>bash deploy.sh production 2>&1 | tee /tmp/pgtest_deploy.log; grep -E "pytest gegen postgresql://nerve_app@/nerve_test|pg_dump --schema-only nerve|Running upgrade 0015 -> 0016|upgrade head" /tmp/pgtest_deploy.log; echo "EXIT=$?"  # Req-1/Req-3-Beleg: Gate lief gegen nerve_test, baute via pg_dump-Restore, upgrade applied nur neue Revs (kein "already exists").</automated>
  </verify>
  <done>
    deploy.sh enthält den Postgres-Gate-Block: Whitelist-Guard + trap cleanup EXIT + Pre-Run-DROP + CREATE
    OWNER postgres + pg_dump --schema-only nerve→nerve_test + alembic_version-Stamp-Dump + alembic upgrade
    head + pytest mit 4 nerve_test-DSNs, jeder Schritt fail-closed mit Klartext-Grund. Deploy-Log belegt
    `pg_dump --schema-only nerve` piped in nerve_test (NICHT create_all), pytest gegen postgresql://…/nerve_test,
    und `Running upgrade 0015 -> 0016` (kein „already exists"-Fehler). anon_worker-PW nirgendwo geloggt:
    `grep ANON_WORKER_TEST_DSN.*nerve_anon_worker /tmp/pgtest_deploy.log` zeigt KEIN Klartext-PW.
  </done>
</task>

<task type="auto" tdd="false">
  <name>Task 2: Dump-Treue-Katalog-Assertion (RLS/FORCE/GRANT + alembic_version) + Schild-Guard-DSN auf nerve_test umlenken (Req-9)</name>
  <read_first>
  ...
  <behavior>
    - Katalog-Treue-Assertion: gegen den im Gate gebauten nerve_test ist nachweisbar, dass der pg_dump die RLS/FORCE/GRANTs treu getragen hat (NICHT nur „Schema da") — das ist der False-Green-Guard. (Die alte A1 „create_all-Ordering"-Verifikation ist MOOT — create_all wird nicht mehr verwendet — und entfällt; diese Task prüft stattdessen Dump-Fidelity.)
  ...
  <action>
    1. **Dump-Treue-Katalog-Assertion (False-Green-Guard, ersetzt die alte A1-create_all-Verifikation):**
       Da das Gate nerve_test am Ende droppt, verifiziere die Dump-Treue durch einen gezielten manuellen
       Build gegen einen Wegwerf-`nerve_test` + Katalog-Query, BEVOR der trap droppt — server-side als postgres:
       ```
       ssh -i ~/.ssh/nerve_vps root@178.104.82.166 'sudo -u postgres psql -c "DROP DATABASE IF EXISTS nerve_test;" && \
         ...
         sudo -u postgres bash -c "pg_dump --schema-only nerve | psql -v ON_ERROR_STOP=1 -d nerve_test" && \
         sudo -u postgres bash -c "pg_dump --data-only --table=alembic_version nerve | psql -v ON_ERROR_STOP=1 -d nerve_test" && \
       ...
       Diese drei Katalog-Werte sind der False-Green-Guard: sie beweisen dass der Dump RLS/FORCE/GRANT treu
       trägt (nicht nur „Tabellen existieren"). Dokumentiere die verbatim-Outputs im SUMMARY.
    ...
    2. **Schild-Guard-DSN umlenken (Req-9/Req-5):** In deploy.sh Z.151 ändere
       ...
       den Gate-Block (als zusätzlichen pytest-Aufruf in dieselbe nerve_app-Gate-Subshell wie pytest — der
       Haupt-`pytest tests/`-Lauf deckt test_schild_guard.py bereits ab, da NERVE_SCHILD_TEST_DSN gesetzt ist).
  ...
  <verify>
    <automated>ssh -i ~/.ssh/nerve_vps root@178.104.82.166 'cd /opt/nerve/app && grep -nE "NERVE_SCHILD_TEST_DSN" deploy.sh'; echo "---"; bash deploy.sh production 2>&1 | tee /tmp/pgtest_deploy2.log | grep -E "test_schild_guard|alembic_version|crm|nerve_test|relforcerowsecurity|nerve_anon_worker"; echo "EXIT=$?"  # Schild-Guard gegen nerve_test grün; crm-Policies≥7 + FORCE + GRANTs da (Dump-Treue).</automated>
  </verify>
  <done>
    Dump-Treue verifiziert + im SUMMARY mit verbatim-Katalog-Output dokumentiert: `pg_policies` crm ≥7,
    `relforcerowsecurity=t` auf allen 5 crm-Tabellen, `role_table_grants` nerve_app=DML + nerve_anon_worker=SELECT,
    `alembic_version`=Repo-HEAD, `\dn` zeigt crm+training (Req-3 End-Zustand). Schild-Guard läuft im Gate gegen
    nerve_test (kein `@/nerve` ohne `_test` mehr in deploy.sh) und ist grün (Req-9). Voller deploy.sh production
    endet grün; Prod-`nerve` unverändert (kein Test-DSN/CREATE/DROP zeigt drauf; pg_dump war read-only).
  </done>
</task>
...
| T-PGTEST-08 | Denial/Spoofing | Gate skippt still bei nicht-provisionierbarer DB → False-Green-Deploy | mitigate | D-07 fail-closed pro Schritt (exit 1 + Klartext-Grund); `psql -v ON_ERROR_STOP=1` bei beiden Dump-Restores; KEIN SQLite/Prod-Ausweich-Zweig; trap droppt nerve_test |
| T-PGTEST-09 | Tampering | nerve_test OWNER nerve_app ODER --no-privileges-Dump → crm-Tabellen owner-bypassen RLS / RLS-Policies fehlen (False-Green) | mitigate | CREATE DATABASE ... OWNER postgres (RESEARCH Q1e/0012-Doku); pg_dump --schema-only MIT owners+privileges (NICHT --no-privileges/--no-owner). **EMPIRISCH BEWIESEN (RESEARCH „⚑ BUILD-PATH LOCKED"):** der Dump trug alle 7 crm-RLS-Policies + ENABLE/FORCE auf allen 5 crm-Tabellen + GRANTs (nerve_app=DML, nerve_anon_worker=SELECT); der echte Cross-Tenant-Test test_tenant_a_cannot_read_tenant_b_account_memory PASSED gegen das dump-gebaute nerve_test (11 passed gesamt) = genuine Isolation, NICHT 0-Zeilen-False-Green. Task-2 re-assertet die Katalog-Werte pro Lauf. |
...
- `crm.*` (account_memory/accounts/contacts/meetings/user_preferences) — RLS/FORCE/GRANTs müssen treu getragen sein
...
|---|---|---|---|
| crm-RLS-Policies | pg_dump --schema-only nerve → nerve_test | `pg_policies` (Katalog) | ✓ 7 Policies bewiesen; Task-2 re-assertet |
| crm FORCE ROW LEVEL SECURITY | pg_dump (mit owner) | `pg_class.relforcerowsecurity` | ✓ t auf 5 Tabellen |
| crm GRANTs (nerve_app DML / anon SELECT) | pg_dump (MIT privileges) | `information_schema.role_table_grants` | ✓ bewiesen; --no-privileges würde es brechen |
| prod-head Stamp-Row | pg_dump --data-only --table=alembic_version | `public.alembic_version.version_num` | ✓ trägt 0015 → upgrade applied nur 0016 |
...
(z.B. 0 crm-Policies nach Dump → --no-privileges/--no-owner versehentlich gesetzt → False-Green-Gefahr → Eskalation, nicht weiterbauen)

<verification>
- Req-1: Deploy-Log zeigt `pytest gegen postgresql://nerve_app@/nerve_test` + `pg_dump --schema-only nerve`.
- Req-3 (End-Zustand-Acceptance, André-autorisierte Mechanismus-Abweichung): Katalog-Query gegen nerve_test
  (Task 2) → `alembic_version`=Repo-HEAD (0016) + crm/training-Schemas (`\dn`) + ≥7 crm-RLS-Policies + FORCE.
...
- Req-7: simulierter Fehler (z.B. falsche Rolle / gestopptes PG / ON_ERROR_STOP) → exit≠0, kein systemctl
  restart, expliziter Log-Grund — MANUELL via Manual-Only-Verification (VALIDATION.md).
...
<success_criteria>
- Gate-Block in deploy.sh: Whitelist-Guard + trap EXIT + Pre-Run-DROP + CREATE OWNER postgres + pg_dump-Restore-Build (schema + alembic_version-Stamp + upgrade head) + pytest 4-DSN, alles fail-closed.
- Dump-Treue-Katalog-Assertion (≥7 crm-Policies + FORCE + GRANTs + alembic_version=head) verifiziert + im SUMMARY dokumentiert; Schild-Guard gegen nerve_test grün.
- KEIN create_postgres_schema.py / stamp 0001 im Gate (kollisionsfrei via pg_dump). anon_worker-PW nie geloggt; alle DSNs → nerve_test; Prod-nerve unberührt.
</success_criteria>
