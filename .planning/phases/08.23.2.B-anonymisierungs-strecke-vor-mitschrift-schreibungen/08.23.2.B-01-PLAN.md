...
must_haves:
  truths:
    - "services/anonymization.py ist importierbar und exportiert AnrufAnonymisierer, anonymize, anonymize_output, register_briefing_pii"
  ...
  artifacts:
    ...
    - path: "services/anonymization.py"
      provides: "Anonymisierungs-Pipeline — drei Funktionen + AnrufAnonymisierer-Klasse"
      exports:
        - AnrufAnonymisierer
        - anonymize
        - anonymize_output
  ...
  key_links:
    - from: "anonymize()"
      ...
      via: "Import in anonymize() — Art-9-Check Schritt 0"
    - from: "anonymize()"
...
<objective>
Erstellt das DSGVO-Foundation-Modul `services/anonymization.py` und die Art-9-Keyword-Liste `services/art9_keywords.py` von Grund auf (Greenfield).
...
- `services/anonymization.py` — AnrufAnonymisierer-Klasse + anonymize() + anonymize_output() + register_briefing_pii() + Fallback-Architektur (D-08)
</objective>
...
<task type="auto">
  <name>Task 2: services/anonymization.py erstellen (AnrufAnonymisierer-Klasse + alle drei Funktionen + Fallback-Architektur)</name>
...
- `AnrufAnonymisierer.mapping` ist dict (RAM-only, nie persistiert)
  ...
  <verify>
    <automated>cd "C:/Users/andre/dev/salesnerve" && python -c "from services.anonymization import AnrufAnonymisierer, anonymize, anonymize_output, register_briefing_pii, is_pipeline_healthy, AnonymizationPipelineUnavailable; print('Import OK'); c = AnrufAnonymisierer(); t1 = c.get_or_assign_token('Mueller', 'PERSON'); t2 = c.get_or_assign_token('Mueller', 'PERSON'); assert t1 == t2 == '[PERSON_A]', f'Stable token failed: {t1} vs {t2}'; print('AnrufAnonymisierer OK')"</automated>
  </verify>

  <acceptance_criteria>
    - `from services.anonymization import AnrufAnonymisierer, anonymize, anonymize_output, register_briefing_pii` schlaegt NICHT fehl
  ...
  <done>services/anonymization.py existiert, alle 4 Symbols importierbar, AnrufAnonymisierer gibt [PERSON_A] zurueck (Buchstaben-Suffix), kein DB-Bezug in database/.</done>
</task>
...
| T-08.23.2.B-06 | Denial of Service | spaCy Modell-Load | accept | Lazy-Load nach Fork; gunicorn ohne --preload; CX22 hat 4GB RAM fuer ~1.4GB Footprint |
</threat_model>

<verification>
Nach beiden Tasks:

1. Import-Check: `python -c "from services.anonymization import AnrufAnonymisierer, anonymize, anonymize_output, register_briefing_pii"` — kein Fehler
...
- services/anonymization.py: alle 4 Symbols + is_pipeline_healthy + AnonymizationPipelineUnavailable exportiert
...
- Exakte Datei-Groesse beider Dateien (wc -l)
