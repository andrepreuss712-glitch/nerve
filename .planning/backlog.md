...
- **★ Zusatz (Fable-Notiz 2026-07-13):** der Advisory-Lock `pg_advisory_xact_lock(819)` in `_migrate_profile_json` ist **xact-scoped** → er fällt beim ERSTEN `conn.commit()` in der Schleife (nicht erst am Funktions-Ende) → theoretischer opener-Doppel-INSERT-Race zwischen Multi-Workern nach dem ersten Commit. PROFILE-MIGRATE-TXN-FIX verbessert die Lage (Entgiftung + Isolation), aber die eigentliche Race-Verschärfung (Lock-Scope / Session-Lock statt xact-Lock, ODER Konsolidierung auf EINEN atomaren Pfad) gehört HIERHER.
- **★ Zusatz (Prod-Realität 2026-07-13):** da der v4-Batch auf PG tot ist und `_migrate_profile_json` `schema_version>=2` skipt, werden **v2/v3-Profile auf Prod NIE auf v4 gehoben** — das echte Heben ist Teil dieser Konsolidierung (Guard a aus PROFILE-MIGRATE-TXN-FIX macht die Lücke sichtbar; aktuell 0 v2/v3 auf Prod, Pre-Execute-Audit).
...
- **🔴 DSGVO-Spannung (NICHT schnell schießen):** berührt den Fundament-Pfeiler „anonymisieren VOR KI-Verarbeitung" ([[04 Entscheidungen/NERVE DSGVO Analyse]]). Roh-Text an die Claude-API (~~AWS Bedrock Frankfurt~~ → **US-direkt**, korrigiert 11.08.) zu geben ist eine bewusste Architektur-Entscheidung, kein Bug-Fix. ⚠ **Die Spannung selbst bleibt unverändert bestehen** — sie hängt am **Roh-Text**, nicht am Server-Standort. Ein Standort-Wechsel löst sie nicht.
...
- **Mechanik-Hinweis (KORRIGIERT 2026-08-11):** ~~der Alt-Score wird beim TAXO2-Noten-Cutover durch die rubrik-basierte Note ersetzt~~ — ⛔ **es kommt keine neue Note.** Die Noten-Engine wurde am 28.06. abgeschafft; der Alt-Score (`app_routes.py`-Formel / `calls.coaching_score`) wird von **METRIK-1** ersatzlos abgelöst durch **Beobachtungen mit Beleg-Zitat + genau EINE Sache fürs nächste Mal**. **Der Kern des Befunds überlebt aber und wird dringlicher:** Es darf **keine stille Leerstelle** geben. Wo zu wenig Material vorliegt, steht sauber **„nicht genug zum Bewerten"** — und zwar aus **Daten-Mangel**, nie als Strafe für ein „Nein".
- **Routing (KORRIGIERT 2026-08-11):** ~~SCORE-UI PFLICHT-PULL zusammen mit TAXO2-Note-Cutover Plan 04~~ → **PFLICHT-PULL für METRIK-1.** ⚠ **Die alte Fassung war gefährlich:** Ein „PFLICHT-PULL" wird beim Planen zwingend gezogen — sie hätte die abgeschaffte Note in die neue Phase zurückgeholt. Verbindlich: `Nerve-Vault/04 Entscheidungen/NERVE Konstrukt - Soll-Verhalten.md` §6.
