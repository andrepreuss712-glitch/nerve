...
**Status:** 📋 **GEPLANT + CROSS-AI EINGEARBEITET 2026-07-31** (4 Pläne, 4 Wellen) → nächster Schritt **Claudian-Pre-Execute-Audit**, danach Execute (Wellen 1-3 in EINER Sitzung wegen des roten Gate-Fensters). Cross-AI Fable: „FREIGABE NEIN“ mit 2 Pflicht-Punkten — **beide eingearbeitet**: **(1)** vierter unerfüllbarer Zähl-Anker (`get('counterpart') or 'gatekeeper'` im vorgeschriebenen Docstring vs. Kriterium „unverändert gegenüber HEAD~1“) gefixt, und die **Planungs-Regel** verankert: *ein Kriterium, das eine Zeichenkette zählt, die der Plan selbst in dieselbe Zieldatei schreibt, ist strukturell unerfüllbar*. Bei der Anwendung dieser Regel auf alle vier Pläne kam ein **fünfter** Fall derselben Klasse zum Vorschein und wurde entschärft (`grep -c "RLock" → 0`, während Modul-Docstring und Assertion-Meldung den „KEIN RLock“-Design-Zwang wörtlich zitieren — der Anker hätte den Executor gezwungen, ausgerechnet die Warnung zu löschen). **(2)** Die **direkteste** Form der eigenen Fehlerklasse (verschachteltes `with <riegel>:` bzw. direktes `<riegel>.acquire()` in bereits gehaltener Region) war unsichtbar UND unbenannt — **geschlossen** über einen zweiten Pfad `_direkte_erneute_nahmen` mit 6 gepaarten Selbst-Test-Fällen. **Dabei ein Rest-Defekt in der Nachbesserung selbst gefunden:** die Positions-Ausnahme `pos == 0` hätte eine **echte** Wieder-Nahme als erste Anweisung einer try-Region still verschluckt — empirisch belegt und entfernt (die dritte Erwerbsform kommt im Produktiv-Code nicht vor: 0 von 5 try-Riegel-Regionen; ihre Falschtreffer-Richtung ist als **Restlücke 7** benannt, laut statt blind). Fable bestätigte unabhängig (dritter Lauf, anderer Ansatz): **47 Nehmer = 42 nur-`with` + 5 nur-`.acquire()`** — das belegt den Boden **45** als beide Halb-Ausfälle fangend —, **99** transitiv, **genau ein** Treffer `claude_service.py:2076`, Fix **verhaltensgleich**. Rot-Lauf-Erwartung unverändert: genau EINE Trefferzeile (Pfad B trägt 0 bei, am Code nachgemessen). Der systematische Sweep (Teil 2 des Auftrags) wurde **vorab von Fable** erledigt und liegt als gesetztes Vorwissen in CONTEXT.md — kein Plan-Task erhebt ihn neu. **Vollständiger Auftrag:** Ende von `.planning/DIALOG-GSD-CLAUDIAN.md` (Claudian, 2026-07-31, „WURZEL GEFUNDEN"). **Sync:** `Nerve-Vault/01 Roadmap.md` parallel gepflegt.
