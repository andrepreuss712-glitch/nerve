...
- **08.23.2.PROFILE-MIGRATE-CONSOLIDATE „Startup-Profil-Migration Konsolidierung" 🔴 ★ AKTIVE PHASE (aufgesetzt 2026-07-16, promoviert aus Backlog `PROFILE-MIGRATE-CONSOLIDATE`; DEFERRED aus PROFILE-MIGRATE-TXN-FIX Schritt 4). Grundlage = Fables gelieferte Diagnose + Fix-Design (Bestand-Audit 14.07.).** Zwei konkurrierende Startup-Profil-Migrationspfade zusammenführen — der TXN-FIX hat nur gehärtet, nicht konsolidiert; die Guards a/b/c aus TXN-FIX sind das Fundament, auf dem diese Konsolidierung aufsetzt.
  ...
  **★ 🔴-nah (Startup-Migration + Transaktions-Semantik + delikater Daten-Code) → Cross-AI (Fable + Gemini) PFLICHT vor Execute (Punkt 24), NICHT auto-advance.** Ablauf: Discuss (kurz, Diagnose steht) → Plan → Fable+Gemini-Review → Claudian-Pre-Execute → Bau → Claudian-Deploy (Zwei-Tore supervised, autonomous:false). Multi-Segment-Gotcha: Pfade hardcoden, gsd-tools umgehen, STATE/ROADMAP hand-editieren.
