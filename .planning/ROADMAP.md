...
**Komplexität:** 🔴 — LLM-Bewertungs-Pipeline + Schema-Umwidmung + DSGVO (Transkript an Sonnet, EU/Bedrock). Cross-AI (Gemini, 3 Sichten) **Pflicht vor Execute**. Real-Daten-Validation Pflicht (halluzinierte-Belege-Check). **Komplexität gegen Leitsatz 2 / Punkt 27 prüfen — der Judge ist das Simpelste-was-geht (schauen+zitieren), KEIN Gewichtungs-Apparat.**
...
- **08.23.2.AUTH-1 „Eingangstür + Netz" 🟡 (SOFORT):** CSRF-Fix A1 (Meta-Token in landing.html + `X-CSRFToken` in 3 Fetches login/register/waitlist + `WTF_CSRF_TIME_LIMIT=None`) + 3 Deploy-Gate-Wächter (`test_signup_journey.py` E2E MIT CSRF-an ERST-ROT-gegen-HEAD; `test_csrf_fetch_guard.py` statischer Sweep state-changing fetch ohne Token→rot, Whitelist 3 exempte; Template-Existenz-Wächter) + Superadmin-Härtung (André-Google-Konto ohne passwort_hash → nur OAuth; SUPERADMIN_EMAIL prüfen) + `_require_admin` liest `g.user.rolle` statt `session['rolle']` (grep alle Vorkommen) + **Warteliste-Rechte-Fix** (`@login_required`+`@superadmin_required` auf `/waitlist/admin` + `/waitlist/invite/<id>`, schwache `flask_session['rolle']=='owner'`-Checks raus — 2 Löcher: fremde Interessenten-PII sichtbar + Owner kann Gratis-EA-Plätze verschenken; Weg a Fable-geprüft; Guard-Test Owner→403) + **changelog.py:84-Sibling-Loch mitgefixt** (André-Direktive 2026-07-04: `@login_required`+`@superadmin_required` auf `/changelog/admin`, kein `@login_required` vorhanden → AttributeError-Pfad geschlossen). Sprach-neutral, keine Abhängigkeit. Liefert Registrierung + PERSID-Test-Konten.
  **✅ GEPLANT 2026-07-04, REPLANNED 2026-07-05 via `/gsd-plan-phase --reviews` (Fable-Cross-AI eingearbeitet; 4 Plans / 4 Wellen strikt serialisiert 01→02→03→04, Pfade hardcoded — Multi-Segment-Gotcha).** Verzeichnis: `.planning/phases/08.23.2.AUTH-1-eingangstuer-netz/`.
  - [ ] 08.23.2.AUTH-1-01-PLAN.md — CSRF-Fix A3 globaler window.fetch-Wrapper (base.html + landing.html, deckt Login/Register/Waitlist + 39 Bestand-Löcher app-weit) + app.py WTF_CSRF_TIME_LIMIT=None + E2E-Wächter test_signup_journey (register-only, ERST-ROT, Cleanup Session→User→Org) (Welle 1)
  - [ ] 08.23.2.AUTH-1-02-PLAN.md — 2 statische Wächter test_csrf_fetch_guard (app-weit rekursiv templates/**+static/**, Vendor-Ausschluss) + test_template_existence_guard, ERST-ROT gegen seeded Verletzung (Welle 2, depends_on 01)
  - [ ] 08.23.2.AUTH-1-03-PLAN.md — Waitlist+Changelog Auth-Härtung (@login_required+@superadmin_required ×3) + Vollständigkeits-Tabelle + test_waitlist_admin_auth (inkl. changelog/admin Owner→403, Cleanup Invitation+invite-Org) ERST-ROT + BLOCKING Prod-Vorab-Verify (Welle 3, depends_on 02, autonomous:false)
  - [ ] 08.23.2.AUTH-1-04-PLAN.md — Rollen-Sweep session→g.user.rolle (5 Routes + 4 Templates, base.html:79 mit g.user-and-None-Guard/BLOCKER 2, profiles-Helper=1 Zeile→18 Caller) + test_rolle_source_sweep-Wächter (routes/** UND templates/**) (Welle 4, depends_on 03)
  **🔴-Marker-Nuance:** Block AUTH ist 🔴, AUTH-1 selbst 🟡 — ABER Auth/Security → **Cross-AI Gemini PFLICHT vor Execute** (CLAUDE.md Punkt 24). NEXT = `/gsd-review --phase 08.23.2.AUTH-1 --all`, NICHT direkt Execute.
- **08.23.2.AUTH-2 „Onboarding-Connector" 🟡:** Router `post_login_destination(user)` an 3 Weiterleitungs-Stellen + `users.onboarding_state`-Spalte (pending/done/skipped, +Bestands-Migration `done`) + BRANCHE_TEMPLATES-Erstprofil-Minimum-Seite. Türöffner für späteren Wizard.
- **08.23.2.AUTH-3 „Kassentür" 🔴 (Cross-AI Pflicht, Geld-Pfad):** pricing.html + Sitz-Stepper + Checkout `quantity` (KEIN trial_period_days) + Webhook-Fixes (`_activate_subscription` echter Status via Retrieve statt hart `active`; `_sync_subscription` quantity→max_users) + `_require_owner` auf checkout/portal + skip_billing/skip_onboarding-Migration. VOR Betriebs-Gate-Scharfschaltung. Prod-Verifikation Stripe (Test/Live, Price-IDs per-Sitz, automatic_tax/US-Steuer).
- **08.23.2.AUTH-4 „Founder/Support-Maske" 🟡:** CustomView im Flask-Admin — org-zentrisch: Firmen→Mitglieder+Monats-Summe (Sitze×Preis); „Testnutzer anlegen" (isolierte Org, NICHT erste Org); skip-Häkchen; Mitglieder entfernen(=deaktivieren/Sitz-frei ≠ DSGVO-Delete)+einladen; Support-Rolle vorbereitet.
- **08.23.2.AUTH-5 „Team/Sitze" 🟡/🔴:** `send_invitation`-Mail + invitations rolle/invited_by/expires_at-Migration + Accept-Sitz-Check + max_users=Einzelwahrheit (organisations.py liest g.org.max_users statt PLANS) + Namen-Erhebung + Portal-Sitz-Verwaltung. W-1..W-4-Wächter.
