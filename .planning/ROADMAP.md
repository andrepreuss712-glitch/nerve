...
- **08.23.2.AUTH-2 „Onboarding-Connector" 🟡:** Router `post_login_destination(user)` an 4 Weiterleitungs-Stellen (S1-S4, D-16) + `users.onboarding_state`-Spalte (pending/done/skipped, +Bestands-Migration `done`) + `users.skip_onboarding` + BRANCHE_TEMPLATES-Erstprofil-Minimum-Seite. Türöffner für späteren Wizard. **✅ GEPLANT 2026-07-08 (3 Plans / 3 Wellen strikt serialisiert 01→02→03). 🔴-nah (Auth-Pfad + Migration 0032 + AUTH-3-Vertrags-Naht) → Cross-AI PFLICHT vor Execute, KEIN Auto-Advance.** Multi-Segment-Gotcha: Pfade hardcoded (`08.23.2.AUTH-2-onboarding-connector`).
  **Plans:** 3 plans
  Plans:
  - [ ] 08.23.2.AUTH-2-01-PLAN.md — Migration 0032 (onboarding_state TEXT+Backfill done+CHECK-Türöffner + skip_onboarding boolean) + Schilder + onboarding_done DEPRECATED-Freeze + Landmine-Stilllegung D-10 (Startup-Flip Postgres-inert, nur Warn-Kommentar) + Migrations-Wächter ERST-ROT. Wave 1.
  - [ ] 08.23.2.AUTH-2-02-PLAN.md — post_login_destination als nummerierte 4-Stufen-Liste (services/onboarding_routing.py) + verbatim AUTH-3-Leer-Slot (D-03/D-04 skip_onboarding≠skip_billing) + Erstprofil-Minimum-Seite (Branche+1 Feld, EXPLIZITES Enum-Mapping D-13, Anlage+aktiv+done selber Commit) + toter dashboard.py-Gate-Rest gelöscht (D-20) + Routing-Matrix-Wächter ERST-ROT. Wave 2.
  - [ ] 08.23.2.AUTH-2-03-PLAN.md — Verkabelung 4 Docks (S1-S4) + S3-JS-Fix landing.html:931 + OAuth-Weiche VOR commit/redirect (D-19) + Skip-Banner Dashboard (D-12) + Wiring-Wächter ERST-ROT + ★ VERBINDLICHE Deploy-Sequenz D-23 (Prod-nerve manuell 0030→0032 VOR Code-Restart + Post-Verifikation 3 User=done, autonomous:false Claudian). Wave 3.
