...
**Plans:** 3 plans / 2 Wellen GEPLANT 2026-07-28 — **3/3 GEBAUT 2026-07-30** (Welle 1 = Plan 01+02, Welle 2 = Plan 03). ⚠ **Phase NICHT fertig:** Plan 04 (Welle 3, Event-Umbenennung + Migration) kommt hinzu und ist noch nicht geschrieben. **Kein Deploy vor Plan 04** — der R3-Waechter ist im Zwischenfenster bewusst rot (ORM 7 Werte vs. DB 9).
...
- [ ] 08.23.2.COUNTERPART-04-PLAN.md — Welle 3: Event-Umbenennung `mode_initial`/`mode_switch` → `counterpart_initial`/`counterpart_switch` inkl. Alembic-Migration (CHECK-Werteliste), UPDATE der 113 Prod-Altzeilen und `models.py` auf die NEUEN Werte (zieht R3 grün) — **noch nicht geschrieben**
