...
>     - **★ ENGPASS ist NICHT die Technik, sondern der FOKUS-KATALOG.** Er ist Vertriebs-Inhalt = **erste Scheibe des US-COACHING-GEHIRNS**. Wird **klein und gleich englisch** gebaut (5-8 Eintraege). Auf Deutsch waere er Wegwerf-Arbeit (gleiches Muster wie die deutsche Schwaerzungs-Whitelist).
...
- [~] 08.23.2.MEHRNUTZER-REST-1-04-PLAN.md — Welle 3: der Fix (`coaching_service.py:8/:59`) + GRUEN-Beleg + Pruning-Notiz/Folgefunde — **CODE-COMPLETE 2026-08-07, GRUEN-BELEG STEHT AUS.** Fix gebaut und gepusht (`e213a2a`): `_analysis_lock` geloescht, `_analysis_lock_for(conv_id)` mit Nehmer-Zaehler, `count()` weiterhin im Riegel, Waechter-Soll `coaching_service.py` 1 → 3 im selben Commit hochgezogen. **Task 2 (`bash deploy.sh production`) ist ein Halt-Punkt fuer den Menschen** (Deny-Regel `~/.claude/settings.json:34-37`) — erwartet `1142 passed, 7 skipped, 5 deselected` + Ist-Zaehlung `services/coaching_service.py: 3`. **Bis zum gruenen Tor blockiert der rote Lauf aus Plan 03 jeden Deploy und der 🔴 START-BLOCKER gilt als offen.** ⚠ Auflage 2: der Fix nimmt die WARTEZEIT, nicht den THREAD-VERBRAUCH — der 50-Nutzer-Fall gehoert zu Fund (2) + Lasttest, nicht hierher. SUMMARY: `08.23.2.MEHRNUTZER-REST-1-04-SUMMARY.md`.
