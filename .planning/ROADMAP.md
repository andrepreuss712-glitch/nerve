...
- **08.23.2.PROFILE-MIGRATE-CONSOLIDATE „Startup-Profil-Migration Konsolidierung" 🔴 ★ AKTIVE PHASE (aufgesetzt 2026-07-16, promoviert aus Backlog `PROFILE-MIGRATE-CONSOLIDATE`; DEFERRED aus PROFILE-MIGRATE-TXN-FIX Schritt 4). Grundlage = Fables gelieferte Diagnose + Fix-Design (Bestand-Audit 14.07.).** Zwei konkurrierende Startup-Profil-Migrationspfade zusammenführen — der TXN-FIX hat nur gehärtet, nicht konsolidiert; die Guards a/b/c aus TXN-FIX sind das Fundament, auf dem diese Konsolidierung aufsetzt.
  ...
  **Plans:** 1 plan / 1 Wave / 8 Tasks (hand-authored, REPLANNED via --reviews 2026-07-16 nach Cross-AI Fable+Gemini)
  - [ ] 08.23.2.PROFILE-MIGRATE-CONSOLIDATE-01-PLAN.md — A1 v4-Batch löschen (EIN lebender Pfad = _migrate_profile_json) + A2 Skip `>=2`→`>=LATEST` (v2/v3→v4, kein Schema-Change) + **A3 GELOCKT = Option 1 Session-`pg_try_advisory_lock(819)` + finally rollback→unlock (H-1 a-d gegen Pool-Lock-Leak; Opt 2 Unique-Index verworfen=mehrere Bausteine je Typ Feature, Opt 3 xact-halten verworfen=rollback gibt Lock frei)** + **A2b `_migration_profile_id`-Injektion (Bug #2, sonst 'Profile ?'/target_id=None)** + **A2c pro-Typ-COUNT (Bug #3, sonst stiller Opener-Verlust)** + B1 Boot-Crash-Netz + B2 created_at + B3 Docstring + B4 6 real-PG-Tests (C/E/F erst-rot). Wave 1. autonomous:false. **★ Cross-AI durch (REVIEWS.md), BAUBEREIT — NEXT: Claudian-Pre-Execute-Audit → Bau (Zwei-Tore, supervised), NICHT auto-advance.**
