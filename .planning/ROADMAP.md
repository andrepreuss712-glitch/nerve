...
>   1. **US-UMZUG** 🟡🔴 — Server-Region USA (Hetzner hat US-Standorte, kein Anbieterwechsel) · Deepgram + ElevenLabs auf US-Zugang · **`services/deepgram_service.py:485` hat `language="de"` HART verdrahtet → mit englischen Calls faellt das Kernprodukt aus** · `services/anonymization.py:40` laedt das deutsche Namens-Modell (`de_core_news_lg`) → US-Namen rutschen im Klartext in die DB · **`routes/payments.py:344` bucht Stripe-Gebuehren nur bei `currency == 'EUR'` → bei Dollar-Abrechnung verbucht die Kostenerfassung STILL NICHTS** (gleiche Fehlerklasse wie die gerade geschlossenen KOSTEN-1-Luecken; in AUTH-3 mitfixen). Gewinn: ~0,2-0,35 s pro Antwort bei ~1 s Budget.
