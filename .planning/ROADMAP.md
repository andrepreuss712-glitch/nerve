...
- **08.23.2.PROFILE-MIGRATE-CONSOLIDATE „Startup-Profil-Migration Konsolidierung" 🔴 ★ AKTIVE PHASE (aufgesetzt 2026-07-16, promoviert aus Backlog `PROFILE-MIGRATE-CONSOLIDATE`; DEFERRED aus PROFILE-MIGRATE-TXN-FIX Schritt 4). Grundlage = Fables gelieferte Diagnose + Fix-Design (Bestand-Audit 14.07.).** Zwei konkurrierende Startup-Profil-Migrationspfade zusammenführen — der TXN-FIX hat nur gehärtet, nicht konsolidiert; die Guards a/b/c aus TXN-FIX sind das Fundament, auf dem diese Konsolidierung aufsetzt.
  ...
  **Plans:** 1 plan / 1 Wave / 6 Tasks (hand-authored, Plan-Checker VERIFICATION PASSED 4 WARNINGs/0 Blocker 2026-07-16)
  - [ ] 08.23.2.PROFILE-MIGRATE-CONSOLIDATE-01-PLAN.md — A1 v4-Batch löschen (EIN lebender Pfad = _migrate_profile_json) + A2 Skip-Schwelle `>=2`→`>=LATEST` (v2/v3→v4, kein Schema-Change, Single-Deploy) + **A3 Advisory-Lock 819 OFFEN (Drei-Wege für Cross-AI, NICHT vorfestgelegt)** + B1 Boot-Crash-Netz + B2 created_at + B3 Docstring (an A3 gekoppelt) + B4 Test schärfen (echter Fehler = Rollback-Isolation). Wave 1. autonomous:false. **★ BAUBEREIT A1/A2/B; A3 wird von Cross-AI+André gewählt → --reviews-Lock vor Execute.**
