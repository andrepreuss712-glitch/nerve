...
**Status:** 🔨 CODE GEBAUT 2026-06-28 (Plan 01–04 komplett + SUMMARY; Plan 05 Task 1+2 Code gebaut). Cross-AI war vor Execute gelaufen (REVIEWS.md, 3 Gemini-Findings eingearbeitet). 16 Dateien, +2770/-167; Migrationen 0028/0029/0030 geschrieben (NICHT angewendet). **D-RESIDENZ (Task 3) ENTSCHIEDEN 2026-06-28: `weiter-direkt`** — Judge nutzt bis Launch den heutigen direkten Anthropic-Weg (Transkript bereits anonymisiert, gleicher Weg wie ALLE Claude-Calls); Frankfurt/Bedrock = eigene spätere Phase für die ganze App. (André bitte in Nerve-Vault/05 Log + Soll-Verhalten nachtragen.) **OFFEN — supervised by André:** (b) finaler BLOCKENDER Deploy (`alembic upgrade 0028→0029→0030` als postgres VOR Restart → `deploy.sh production` pytest-Gate → Cold-Call-Live-Test + Compliance-Test + Fuzzy-halluzinierte-Belege-Check). **Phase bleibt OFFEN bis Live-Test grün** (CONTEXT-Constraint Plan 05). NICHT autonom abgeschlossen (HART Kein-Local-Dev / STOP-VOR-DEPLOY). Multi-Segment-Gotcha: Pfade hardcoded, gsd-tools umgangen, STATE/ROADMAP hand-editiert. CONTEXT.md neu für den Judge; alter Plan 02 (Anker) als SUPERSEDED-Grabstein, neu 02–05.
...
- [~] 08.23.2.TAXO2.HANDLING-TIMING-05-preview-und-checkpoint-PLAN.md — Preview „Beobachtung statt Zahl“ + Fuzzy-beleg_check (Task 1+2 GEBAUT 2026-06-28) — OFFEN: finaler BLOCKENDER supervised Deploy/Live-Test/halluzinierte-Belege-Check (Task 4, autonomous:false). D-RESIDENZ (Task 3) ENTSCHIEDEN = weiter-direkt. [W5]
...
- [ ] 08.23.2.TAXO2.HANDLING-TIMING-07-transcript-resolved-setpoint-PLAN.md — GAP-FIX (Live-Befund: transcript_resolved nie True gesetzt — Plan-01-Code-Hälfte fehlte → Bewerter/Übernahme liefen nie, keine rubric_score v2). EINE unbedingte Zeile `_call_row.transcript_resolved=True` in api_beenden vor dem calls-commit (~app_routes.py:725, resolved-als-absent, spiegelt audio_health_resolved) + 2 Runtime-Tests; bestehender slow_lane.put-Anstoss passt dann das _call_end_merge-Gate. Kein Migration/Service-Change (DB auf 0030). gap_closure, GEPLANT+plan-checker PASSED 2026-06-28 (Punkt-14 Exit-Pfad-Audit: kein ended-Pfad ohne Setpoint). NÄCHSTER SCHRITT: Pre-Execute-Audit → Execute (--gaps-only) → Re-Deploy (Code+Gate) → Live-Test [W7]
