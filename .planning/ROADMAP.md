...
### Phase 08.23.2.MEHRNUTZER-REST-1: Lernkarten-Erzeugung — globaler Riegel um den LLM-Aufruf raus, Riegel pro conv_id rein (INSERTED 2026-08-06) 🔴 START-BLOCKER — ✅ **ABGESCHLOSSEN + LIVE 2026-08-07**
...
- **Ins Leere gingen:** Verklemmung (kein Pfad haelt den Ablage-Riegel ueber I/O, keine Rekursion) · die Reihenfolge release-vor-Dekrement inkl. `is eintrag`-Identitaetspruefung · **das von GSD ehrlich benannte Restfenster** zwischen `acquire()`-Rueckkehr und `erworben = True` (unter gthread gibt es keine asynchronen Thread-Interrupts; ein Gunicorn-Timeout toetet den ganzen Worker und verwirft den Speicher ohnehin).
