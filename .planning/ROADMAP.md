...
- **08.23.2.PROFILE-MIGRATE-TXN-FIX „Startup-Profil-Migration Transaktions-Härtung" 🔴 (NEU 2026-07-13, promoviert aus Backlog `STARTUP-BATCH-MIGRATION-TXN`):** Die zwei Startup-Migrationspfade vergiften bei jedem Start eine Postgres-Transaktion → versions-lose Profile bleiben unmigriert (`[Schema]`/`InFailedSqlTransaction`, Fingerabdruck „Profile ?"). **Root-Cause (Fable+Claudian Code+Prod-verifiziert):** totes `ALTER TABLE profile_opener ADD COLUMN type` (`_migrate_profile_json` app.py:1423, `except:pass` OHNE rollback) → Spalte existiert → DuplicateColumn → vergiftete conn → Folge-Statements sterben still (Postgres-only). **Scope = Option A + Guards (LOCKED):** (1) Mine raus, (2) `conn.rollback()` in jedem swallowing-except, (3) Pro-Profil-Savepoint (`begin_nested`) im v4-Batch, + Deferral-Guards (a) Post-Batch-Regression-CRITICAL-Log / (b) Skip-if-schon-migriert (kein v4→älter-Ping-Pong) / (c) Tech-Debt-Doc-Block. **★ Cross-Layer-Korrektur (Punkt 21/22):** `profiles` hat KEINE `schema_version`-Spalte (lebt im daten-JSON) → Guards lesen JSON, kein Spalten-SELECT. + CLAUDE.md-Regel „nie except:pass auf PG-conn ohne rollback" + Erst-Rot-Regressions-Wächter (real-PG). **Plan:** 1 Plan / 1 Wave / 6 Tasks (5 auto + Deploy/Live-Verify-Checkpoint), autonomous:false. **★ 🔴 (Startup-Migration + TX-Semantik) → Cross-AI PFLICHT vor Execute (Fable + Gemini, Punkt 24), NICHT auto-advance.** Konsolidierung der zwei Pfade DEFERRED → Backlog `PROFILE-MIGRATE-CONSOLIDATE`.
  - [ ] 08.23.2.PROFILE-MIGRATE-TXN-FIX-01-PLAN.md — Mine raus + rollback-Hygiene + Savepoint + Guards a/b/c + CLAUDE.md-Regel + Erst-Rot-Wächter. Wave 1. autonomous:false (Deploy/Live-Verify-Checkpoint). **PRE-REVIEW-Stand — erst Cross-AI, dann Execute.**
