...
>   - **★ SCHRITT 4.0.4 SPRACHE-1 — die Erkennung umschaltbar machen (NEU 16.08., Andre-Entscheidung) · 🟢 klein · Position: dieselbe Mini-Runde, direkt hinter 4.0.3.**
>     **🔴 ANLASS, an echten Daten belegt (16.08.):** Andres drei **englische** Testanrufe fuer den METRIK-1-Fokus-Katalog haben **kein verwertbares Transkript** erzeugt — zwei gar keins (`poor_audio_health`, 0 Redeabschnitte, Sprechzeit NULL), einer 640 ms. **Ursache am Code: `services/deepgram_service.py:583` — `language="de"` hart im Verbindungs-Aufbau.**
>     **Folge: Der Fokus-Katalog aus METRIK-1 ist bis dahin NICHT belegbar** — er ist englisch, und wir koennen keinen englischen Anruf aufnehmen. Die beiden Abnahme-Punkte („drei englische Test-Anrufe", „Fokus feuert") bleiben in METRIK-1 **offen mit Termin hier**.
>     ⚠ **NICHT der grosse Englisch-Umbau (US-B).** Die Sitzung kennt ihre Sprache bereits (`:836`, aus der Nutzlast mit Vorgabe `de`) — **die Live-Verbindung reicht sie nur nicht durch.** Umfang: Durchreichen + ein Schalter, wo Andre die Sprache setzt. Modell `nova-3` kann Englisch. **Bewusst NICHT dabei: Schwaerzung, Oberflaeche und Bewerter-Auftrag bleiben deutsch — das ist US-B.** Fuer Testanrufe mit erfundenen Namen reicht das.
...
>       **★★★ NACHTRAG 3 — 13.08., GSD-Fund am Code + fuenf Andre-Entscheidungen (Bereich 4 „was ersetzt die Note").** **Die „vier Leser" waren nur die Leser von `_calc_call_score`. DREI WEITERE Bildschirme zeigen eine Gesamtnote direkt aus `kb_end`:** Wochen-Dashboard-Kachel (`performance.py:101-102` → `dashboard.html:121-123`) · Wochen-Dashboard-Begruessung (`performance.py:259-268`) · „Zuletzt im Training: Ø 78/100" auf der Live-Auswertungsseite (`app_routes.py:1112-1135` → `session_detail.html:382`). **⚠ KEIN Anker auf `_calc_call_score` findet sie. Vierer-Liste → SIEBENER-Liste.**
>       **★ FUND UNTER DEM FUND (Claudian, am Code nachgeprueft):** Das Wochen-Dashboard mittelt `kb_end` ueber Live-Anrufe und nennt es **„Ø Score"** — in Live-Anrufen steht dort die **KAUFBEREITSCHAFT DES KUNDEN**. **Auf dem meistbesuchten Bildschirm steht die durchschnittliche Kauflaune der Kunden als Leistung des Verkaeufers.** Dieselbe Krankheit wie auf der Auswertungsseite, nur nie dort gesucht. *(Geprueft und WIDERLEGT: keine Vermischung Training/Live — `logs_90` filtert auf `typ == 'live'`, `performance.py:56-62`.)*
>       **➡️ FUENF ENTSCHEIDUNGEN:** **① PiP** (`pip-launcher.js:4287-4302`): Note raus, **Zaehl-Kacheln bleiben** + Knopf „Auswertung ansehen" — **kein Nachladen, kein Wartekreisel** (latenz-empfindlichster Moment). **② Trend-Streifen: ERSATZLOS RAUS**, ⛔ keine neutrale Ersatz-Zahl (misst Betriebsamkeit statt Fortschritt) — **Ort+Termin: kommt zurueck mit der Fokus-Serie / Requirement 8. Vertagt, nicht gestrichen.** **③ Score-Hero raus**, KI-Einschaetzung rueckt nach oben. **④ Vierer-Aufriss ganz raus** (⛔ „Zahlen bleiben" verworfen — liesse die 100-%-Redeanteil-Zeile stehen, Termin 4.0.2). **⑤ Gap C: Training BEHAELT seine Note, Hero wird typ-abhaengig** (`training.py:721`, `dashboard.py:794`, neue Form hart auf `typ=='live'` in `session_detail.html:125` → Hero blind entfernen laesst die Trainings-Note **still sterben**). **Begruendung der Ungleichbehandlung:** Die Note war falsch, weil sie im echten Anruf die **Kundenlaune** mass; im Training gibt es ein festes Szenario mit richtiger Antwort — dort misst sie tatsaechlich den Verkaeufer. **★ Admin-Sicht (`admin_ewb.py:72`) BLEIBT** (misst Datenqualitaet, benotet keinen Verkaeufer).
>       **⛔ ABNAHME PRAEZISIERT: „kein LIVE-Anruf zeigt eine Gesamtnote"**, nicht „kein Bildschirm" — Letzteres waere per Bauart nie gruen (Bau-Regel 19 Punkt 8).
>       **★★★ NACHTRAG 2 — 13.08., nach ECHTER Fable-Gegenlesung am Code (41 Pruefschritte). GILT VOR allem darunter. Volltext im SPEC-Kasten „NACHTRAG 2" + Fragen-Kanal.** ⚠ **Der Fable-Durchgang vom 11.08. wurde abgebrochen und kam nie an — Claudian hatte Befunde trotzdem als „von Fable belegt" weitergegeben. Code-Aussagen nachtraeglich selbst geprueft und korrekt, Quellenangabe erfunden. Zurueckgenommen: „Freitext waere spaeter abbildbar" (Gemini belegt das Gegenteil).**
>       **① 🔴 DER FOKUS-KATALOG KANN AUF DEM GESAMTEN BESTAND NIE ANSCHLAGEN.** Die vier Wortlisten pruefen **englische** Woerter — **alle 87 Anrufe sind deutsch.** Folgen: die Abnahme „Kriterium nachweisbar verletzt" ist auf echtem Bestand **unerfuellbar**, und der Fall **„kein Kriterium verletzt" war nirgends geregelt**, obwohl Requirement 5 „genau EINE Sache" verlangt — **er ist der Normalfall.** **➡️ ANDRE 13.08.: Katalog bleibt ENGLISCH, Abnahme ueber DREI ENGLISCHE TEST-ANRUFE, plus „diesmal nichts"-Zweig bauen.**
>       **② 🔴 DIE „EINE SACHE" WAEHLT DER CODE, NICHT DAS MODELL — Requirement 6 neugefasst.** Modelle **zaehlen notorisch schlecht**; der Rechenweg zieht den Beleg direkt aus dem Transkript (per Bauart echt). **K.-o.-Grund:** Das erzwungene Antwort-Schema **MUSS** einen Schluessel liefern → bei einem guten Anruf muesste das Modell einen Verstoss **erfinden**. Vier feste englische Satz-Schablonen, kein zusaetzlicher KI-Aufruf; Modell behaelt nur die Kopfzeile. **⛔ Abnahme-Kriterium (d) GESTRICHEN** (prueft sich selbst) → **Ersatz: Zaehlregel-Tests, erst rot**, inkl. „ab 4×"-Schwelle exakt auf der Kante.
>       **③ 🟢 ANZEIGE-SPERRE — der Kommentar „(Leakage-Schutz)" ist ein ETIKETTEN-IRRTUM.** Belegter Ursprung: **Andres Mai-Direktive** (`backlog.md:191-198`, `D.UX.4-CONTEXT.md:77`), Zweck Daten-Qualitaet. **Bewerter-Schutz ist es NICHT** (ergebnis-blind, fertig bevor die Sperre greift). **Sie hat belegbar nie einen Klick erzeugt** — conv 264/266 blieben **trotz** Sperre unbestaetigt. **➡️ ANDRE: nur die Sperre auf der AUSWERTUNGSSEITE raus, Pflicht-Wahl im Anruf-Fenster bleibt.** Unbestaetigte Anrufe **markieren** + aus der Korrelation nehmen.
>       **④ ⚙️ SUBSTANZ-TOR, drei Zusatzregeln:** gemischte NULL-Werte **im Zweifel durchlassen** (`models.py:969`: Text ohne Wortobjekte → NULL obwohl gesprochen) · **Redeabschnitt = Zeile mit `word_count IS NOT NULL`** (loest die EWB-Kippe: sonst passiert ein Satz + ein Knopfdruck als „2 Abschnitte") · **nur BERATER-Woerter zaehlen**.
>       **⑤ ⚙️ EWB-PSEUDO-SATZ — Korrektur an Nachtrag 1 ③: NICHT aus der Nutzer-Anzeige entfernen** (bewusste Alt-Entscheidung, `deepgram_service.py:1169-1174`). **Nur zwei Filter: Bewerter-Auftrag + Pruef-Korpus.** Zeilenangabe war veraltet, heute `:1193-1206`.
>       **⑥ ⚙️ Requirement 8 war SACHLICH FALSCH** — es verglich gegen die blinde Beobachtung, die **keine Wortlisten-Zaehlungen** enthaelt. **Neu: zaehlt gegen das TRANSKRIPT.**
>       **⑦/⑧ ⚙️** Zaehler der verworfenen Beobachtungen bekommt **Ort + Leser** (`payload_jsonb` + Admin-Sicht, UPSERT-Semantik festlegen) · **Zitat-Pruefer prueft gegen exakt denselben gerenderten Text wie der Bewerter** (gemeinsamer Renderer), sonst hausgemachte Beinahe-Treffer.
>       **⑨ 🟡 Fund mit Termin, NICHT hier:** Wochen-Dashboard coacht weiter „Redeanteil unter 40 %" (`dashboard.py:283-300`) → **Termin 4.0.2**.
>       **★★ NACHTRAG 11.08. — vier Befunde aus Claudians Gegenlesung der SPEC.md, Andre-freigegeben. Volltext im Nachtrag-Kasten der `SPEC.md` + im Fragen-Kanal.**
>       **① REIHENFOLGE: Zitat-Pruefer VOR dem Tor-Umbau** (die SPEC hatte es umgedreht). Kanonisch seit 03.08. Geht das Tor zuerst live, sind bis zum naechsten Ausrollen **alle** Zitate ungeprueft sichtbar. **Zulaessig: beides in EINEM Ausrollen. Verboten: Tor live, Pruefer spaeter.**
>       **② FOKUS-KATALOG v1 = NUR die vier Wortlisten-Punkte (A).** **C (Live-Symbol) faellt raus** → widerspricht der eigenen Abgrenzung „Live-Bildschirm = out of scope", taugt nicht als „eine Sache fuers naechste Mal", Kriterium (Reaktionszeit) ist seit 03.08. vertagt. **→ neuer Ort: Punkt 4c.** **B (vier Zeitmasse) zurueckgestellt** → braucht Sprechtempo + Redeanteil, **beide heute messbar falsch** (`get_speech_stats` zaehlt Pausen als Sprechzeit, 14 % daneben; Redeanteil hat drei widersprechende Wahrheiten und ist im cold_call baubedingt 100 % → nur ueber die Ersatz-Rechnung *Sprechzeit ÷ Spanne* gueltig). Korrektur steckt in **4.0.2**, das **nach** METRIK-1 laeuft. **→ neuer Ort: direkt hinter 4.0.2.** ⚠ Im SPEC kam „Tempo" **null Mal** vor. **Das ist exakt der Rueckfallweg, den Andre am 11.08. freigegeben hatte — jetzt als Vorgabe statt als Notausgang.**
>       **③ NEU IN SCOPE — verlorener Auftrag wiedergefunden: der EWB-Pseudo-Satz.** `deepgram_service.py:1097-1105` schreibt beim Knopfdruck eine echte `transcript_segments`-Zeile mit **`speaker=1` (KUNDE)** und Text `"<typ> *ewb button*"`. **Zu pruefen: landet sie im KI-Text und/oder in der Nutzer-Anzeige?** Falls ja, haelt der Bewerter einen Knopfdruck fuer eine Kundenaeusserung — **und `beleg_check` wuerde ihn als gueltiges Zitat BESTAETIGEN, nicht abfangen.** Deshalb **vor** dem Pruefer-Anschluss klaeren. Die Roadmap verankert das seit 10.08. ausdruecklich hier („Termin = hier, nicht irgendwann"); die SPEC hatte es verloren.
>       **④ Requirement 8 (Anwendungs-Pruefung) wird LETZTE Welle** — notfalls abtrennbar. Bleibt in Scope, sonst haette der Fokus-Schluessel keinen Abnehmer (toter Ausgang, R2).
...
> **★★ HIER EINGEHAENGT 14.08. (Andre-Entscheidung, Termin = dieser Punkt): 👁 DIE KAUFBEREITSCHAFT MUSS AUS DER LERNKARTE RAUS.** Der Lernkarten-Auftrag (`coaching_service.py:18`, METRIK-1 Restluecke R-1) bekommt weiterhin `kb_start`/`kb_end` und kann daraus **Fliesstext ueber die Kauflaune des Kunden** bauen — den der Verkaeufer **nach jedem Anruf liest**. METRIK-1 entfernt bewusst nur Verbraucher, die eine **Zahl** zeigen; diese Stelle produziert **Prosa** und rutscht durch jede Zahlen-Suche. **Gefunden von Gemini (Cross-AI 14.08.), von GSD offen protokolliert statt still eingebaut, von Andre abgenickt.** ⚠ **Warum genau hier:** Dieser Punkt streicht den **Erzeuger** — solange der lebt, waere das Entfernen an der Lernkarte ein zweiter Eingriff an derselben Stelle. **Warum nicht spaeter:** Bis dahin liest Andre bei jedem Testanruf weiter die Kundenlaune als Rueckmeldung ueber sich selbst.
...
| 3 | ✅ `08.23.2.METRIK-1-03-PLAN.md` **AUSGEFUEHRT + AUSGEROLLT 2026-08-15** (3/3 Tasks, 6 Commits `b29c57c` · `a72da8b` · `61a7029` · `a580461` · `9b4e322` · Abschluss) | Anzeige vom Bestaetigungs-Klick entkoppelt + dritter Ablehnungs-Zweig. ⛔ Zweig (0) **ersatzlos geloescht** — die Sperre hat belegbar **nie einen Klick erzeugt** (`conv 264`/`conv 266` blieben trotz Sperre unbestaetigt); an ihrer Stelle eine **ehrliche Markierung**. Pflicht-Wahl im Anruf-Fenster **unangetastet** (`static/pip-launcher.js` nicht im Diff). Alt-Zweig `too_few_high_confidence_events` **lebt weiter** (Alt-Zeilen in der DB), festgenagelt von einem Regressions-Test. **Korrelations-Feststellung:** heute existiert **keine** Korrelations-Rechnung (Sweep 0 / gepaarter Existenz-Anker 25) — die Markierung ist der Haken fuer die spaetere. ⚠ Ein Plan-Anker war eine **Fehlmessung** (`inspect.getsource` steht in einem Bestands-Docstring; Code-Form-Anker = 0) — als falscher Sollwert dokumentiert, **kein Code gebogen**. ❌ Offen (UAT, blockiert nichts): die drei Browser-Pruefungen. SUMMARY: `08.23.2.METRIK-1-03-SUMMARY.md` | SPEC-R4 | ✅ **Ausrollen 2 GRUEN** — Deploy-Tor **1220 passed / 0 failed**, Exit-Code 0, Neustart erfolgt, keine Migration; die Zahl war **vorhergesagt** (1200+5+15) |
| 3 | ✅ `08.23.2.METRIK-1-04-PLAN.md` **AUSGEFUEHRT 2026-08-15** (3/3 Tasks, 5 Commits `d343589` · `2340595` · `43f6e66` · `90af06c`) | Fokus-Katalog v1 (4 Wortlisten-Punkte, englisch, **vom Code** gezaehlt und gewaehlt — kein Modell in Auswahl oder Formulierung, mechanisch belegt). Server-Lauf `triage.sh` **15 passed**; ROT davor **8 failed / 7 passed** gegen eine absichtlich auf Teilstring zurueckgedrehte Zaehl-Zeile (nie committet) — dabei zaehlte `we_not_i` auf einem **deutschen** Anruf **26** Ich-Treffer, drei Fehlerklassen in einer. 👁 **Andre 15.08.: `riegel`** — A1 (`reason_for_call`) bleibt mit Sprach-Riegel, Katalog behaelt **vier** Punkte; benannte Grenze: Heuristik, kein Spracherkenner. ⚠ Zwei Plan-Anker waren **Fehlmessungen** (Wortgrenzen-Anker: `\w` ist in GNU-grep eine Zeichenklasse; Fragen-Kanal-Anker: die Plan-Pruefung kuendigt die Frage selbst im Fliesstext an) — als falsche Sollwerte dokumentiert, **kein Code gebogen**. SUMMARY: `08.23.2.METRIK-1-04-SUMMARY.md` | SPEC-R6, SPEC-R7 | ⛔ **KEIN Deploy** — null Aufrufer, kommt in Plan 05 (Bau-Regel 20 terminiert) |
| 4 | ✅ `08.23.2.METRIK-1-05-PLAN.md` **AUSGEFUEHRT 2026-08-15** (3/3 Tasks, 6 Commits `203416a` · `5c9cbc4` · `a706b46` · `6cfc02d` · `f261794` · `85070cd`) | Form 2 Backend: **belegte Kopfzeile aus dem SELBEN KI-Aufruf** (zwei zusaetzliche Felder im bestehenden Tool-Schema, kein zweiter Auftrag) + **genau EINE vom Code gerechnete Sache**, beide in derselben `rubric_score`-Zeile (`_kopfzeile` / `_fokus`) und im Anzeige-Kontext **beider** Render-Zweige. ★ **`services/fokus_katalog.py` hat seinen einzigen Produktiv-Aufrufer** — die in Plan 04 terminierte Bau-Regel 20 ist eingeloest. Server-Lauf mit den **Deploy-Markern**: **1234 passed / 7 skipped / 5 deselected** (rechenbar 1220+6+6+2); **drei ROT-Belege verbatim**, alle mit erreichtem Pruefkatalog — darunter der Fund, dass die Kopfzeile ohne diesen Plan ueber die Vorwaerts-Vertraeglichkeits-Regel **an der Zitat-Pruefung vorbeilief**. ⛔ Ungleichbehandlung bewusst: `no_match` loescht die **GANZE** Kopfzeile (Lob ohne Beleg), waehrend bei `_compliance` das Flag stehenbleibt; `_fokus` wird **IMMER** geschrieben, auch mit `focus_key=None` (D-10). 🔎 Firmennamen-Quelle **belegt statt geraten**: die drei `firma`-Treffer in `models.py` sind alle die Firma des **Interessenten** — richtig ist `organisations.name` ueber `users.org_id`. ⚠ Ein Plan-Anker war eine **Fehlmessung** (`DIMENSIONS_VERSION = 2` trifft auch einen Bestands-Kommentar) — als falscher Sollwert dokumentiert, **kein Code gebogen**. 📌 **Punkt 23: Schild-Schuld benannt** — `observations_jsonb` bekommt sein Schild in **Plan 07 Task 3 (Migration `0041`)**, im selben Ausrollen. ❌ Nicht bewiesen: kein echter Anruf, keine `api_cost_log`-Gegenprobe, fuer keinen Nutzer sichtbar (Template liest die Werte erst in Plan 07). SUMMARY: `08.23.2.METRIK-1-05-SUMMARY.md` | SPEC-R5, SPEC-R6 | ⛔ **KEIN Deploy** — 05/06/07 rollen gemeinsam in Plan 07 |
| 5 | ✅ `08.23.2.METRIK-1-06-PLAN.md` **AUSGEFUEHRT 2026-08-15** (3/3 Tasks, 7 Commits `4a75e79` · `e12853d` · `63bdc4f` · `01ee23a` · `efd1fe2` · `7afbe40` · `e837be7`) | Auswertungsseite: alte Note raus, Gap C. **Ein LIVE-Anruf zeigt keine Zahl mehr, die wie eine Note aussieht** — Score-Hero, Vierer-Aufriss (**GANZ**, inkl. der immer-100%-Redeanteil-Zeile), Trend-Abzeichen (**ersatzlos, auch fuer Training**), „Result: X/100" im Kopfbereich und der Trainings-Notendurchschnitt (Lesestelle 7) sind gefallen. ⭐ **Gap C gebaut, nicht angenommen:** der Hero bekam eine **NEUE** `conv.typ == 'training'`-Weiche — vorher hatte er **keine**; Training behaelt seine Note, laufzeit-belegt (`test_training_behaelt_seine_note`). Server-Lauf mit den **Deploy-Markern**: **1239 passed / 7 skipped / 5 deselected** (rechenbar 1234 + 5); **zwei ROT-Belege verbatim**, der zweite **neu gezogen**, weil der erste nur auf dem Existenz-Anker rot war (mehrdeutig = kein Beleg). ★ **Bau-Regel 20 belegt, warum die alte Formel HIER nicht faellt:** acht Vorkommen in `routes/dashboard.py`, davon **zwei** mit Leerzeichen bzw. `fuer` dahinter — sie entkommen jedem klammer-gebundenen Anker und haetten **Plan 08 per Bauart unerfuellbar** gemacht; alle acht sind weg (`grep -c` **ohne** Klammer = 0), der **zweite Leser** (`app_routes.py:1375`) ist benannt und faellt in Plan 08. ★ **NEUER FUND — eine ZEHNTE Lesestelle:** der Empty-State-Satz in Sektion 13 („Aktuell siehst du den Gesamt-Score …") stand im **LIVE**-Zweig, war nach dem Umbau **unwahr** und sperrte den eigenen Abwesenheits-Anker — ersatzlos gestrichen. 🔎 **Realdaten (T-METRIK1-06-02):** `conversation_logs.typ` hat auf Prod **genau einen** Wert (`live`, 78 Zeilen) — **kein dritter Typ**; ❌ aber auch **null** `typ='training'`-Zeilen, Gap C ist **nicht an Realdaten** belegt. ⚠ **Drei Plan-Anker waren Fehlmessungen** (Nachbarschafts-Anker `-A2` statt `-A3` — der Plan widerspricht sich zwischen `<action>` und `<acceptance_criteria>`; stale `.pyc` im `trend_avg`-Anker; das nackte Wort `Redeanteil` traf eine legitime Uebungs-Empfehlung derselben Seite) — als falsche Sollwerte dokumentiert, **kein Code gebogen**. 📌 **Punkt 23: keine Schild-Schuld** (nur Anzeige-/Rechenpfade, kein Schema). SUMMARY: `08.23.2.METRIK-1-06-SUMMARY.md` | SPEC-R9 | ⛔ **KEIN Deploy** — 05/06/07 rollen gemeinsam in Plan 07 |
| 6 | ✅ `08.23.2.METRIK-1-07-PLAN.md` **AUSGEFUEHRT 2026-08-16** (6 Commits `f1f33c6` · `22893c2` · `2cb1b63` · `9ebcdd7` · `db3c189` · `4d3c2d1`) | Form 2 sichtbar: **eine belegte Kopfzeile** ganz oben, **genau EINE Sache** fuers naechste Mal (zaehlender Test `html.count(satz) == 1`, nicht bloss „vorhanden"), vollstaendige Beobachtungen hinter `<details>`/`<summary>` (**Bordmittel — kein neues JS, keine neue CSS-Klasse**, `git diff $BASIS -- static/nerve.css` leer), ehrlicher Zweig **„Nothing flagged this time."** als **NORMALFALL** (D-10), KI-Karte an die **erste** Stelle geschoben (D-15, nur verschoben, nicht umgeschrieben). **Migration `0041`** zieht das Schild von `observations_jsonb` nach (Punkt 23, Schuld aus Plan 05 eingeloest): nennt den **zweiten Schreiber** `slow_lane.py` und **alle drei** Unterstrich-Schluessel; ORM/DDL per **Byte-Vergleich** zeichengleich; **gegen ein echtes Postgres gelaufen** (`Running upgrade 0040 -> 0041`, Doppelpunkt-Falle gemessen nicht getroffen). Server-Lauf mit den **Deploy-Markern**: **1246 passed / 7 skipped / 5 deselected** (rechenbar 1239 + 6 + 1); **drei ROT-Belege verbatim**. ★ **NACHTRAG (Andre 16.08.): die Redeanteil-Norm fliegt ERSATZLOS aus dem Bewerter-Auftrag** — die KI ruegte den Nutzer woertlich fuer „0 % Redeanteil … weit unter der Norm von ~45 % Kunde", obwohl wir den Kunden im cold_call **baubedingt gar nicht hoeren** (der Wert ist eine **Konstante, die wie eine Messung aussieht**). ★★ **Und die Norm hatte einen ZWEITEN TRAEGER**, den der Auftrag nicht kannte: `judge_dimensions.py:112/117` rendert ueber `dimensions_for_prompt()` in **denselben** Prompt — **gefunden nur, weil der Anker auf das GERENDERTE ARTEFAKT zeigt statt auf die Datei**; ein datei-gebundener `grep` haette gruen gemeldet (Punkt 31). **Kein Ersatzwert**; Ersatz-Rechnung (Sprechzeit ÷ Spanne) **definiert und terminiert → 4.0.2**. ⚠ Zwei Plan-Anker waren **Fehlmessungen** (`<details`/`<summary` zaehlen den vom Plan selbst vorgeschriebenen Jinja-Kommentar mit; `exec_driver_sql`/`op.execute(` den geerbten `_comment`-Docstring — Gegenprobe: `0039`/`0040` liefern dieselben Zahlen) — als falsche Sollwerte dokumentiert, **kein Code gebogen**; damit **dreizehn** Instanzen der Klasse in dieser Phase. ✅ **Plan 02 ist WIRKUNGS-BELEGT und geschlossen** (`conv 272` judged **ohne EWB-Knopf**, letzte Bewertung davor 01.08.; `beleg_check` geprueft 4 / verworfen 1). ❌ **Zwei Abnahme-Punkte in dieser Phase NICHT ERFUELLBAR** — „drei englische Test-Anrufe" und „der Fokus feuert": `deepgram_service.py:583` traegt `language="de"` **hart**, englische Anrufe erzeugen **kein Transkript**. **Offen mit Termin → 4.0.4 SPRACHE-1.** Der Fokus-Katalog ist damit gebaut, getestet, verdrahtet — **und an keinem echten Anruf je ausgeloest.** SUMMARY: `08.23.2.METRIK-1-07-SUMMARY.md` | SPEC-R5 | 🔴 **Ausrollen 3 UNVOLLSTAENDIG — Migration ja (`inspect.sh migrations` → `0041`), CODE NEIN.** Belegt: letzter Neustart **2026-08-15T20:04:47Z**, Master+Worker seit **Aug 15 20:04:46** unveraendert, `deploy.sh:127` = `systemctl restart nerve`; `slow_lane.py` ist auf der Platte **18 Min juenger als der Prozess** → im laufenden Prozess **nicht geladen**. Laufzeit-Gegenprobe passt exakt (`conv 272` hat `_kopfzeile`, aber **kein** `_fokus`, obwohl `slow_lane.py:923` es **bedingungslos** schreibt). **`bash deploy.sh production` steht aus** — vorhergesagt **1246 passed** |
| 7 | 🔵 `08.23.2.METRIK-1-08-PLAN.md` **GEBAUT 2026-08-18, NICHT AUSGEROLLT** (Tasks 1+2+2b von 3, 5 Commits `b8cd77f` · `2db86be` · `dff1d66` · `0ac846c` · `90876bd`) | Anruf-Fenster ohne Note + Trend-Streifen **ersatzlos** (an beiden Stellen) + beide Formel-Kopien (`_calc_call_score`, `_calcScore`) und `/api/postcall/trend` geloescht. ★ **Der Knopf „Auswertung" wurde AUS der Noten-Klammer gezogen** — wer nur die Note entfernt, entfernt den Knopf mit; alle **sechs** `nlp-btn-details`-Stellen gelesen, **fuenf unveraendert** (eine davon **deaktiviert** den Knopf bewusst). 📌 **Token-grep entschied gegen die Vermutung:** die drei `--session-score-*` **bleiben** (`.n-session-score-*` nutzt sie weiter). ★★ **PFLICHT-NACHTRAG 17.08. mitgebaut (Task 2b, Bestandsfehler):** nach „Nächster Call" stand der **ganze** vorige Anruf noch im Fenster (vier Anrufe, vier Dauern, **eine** Anzeige; beim vierten ein Ausgang, den die DB gar nicht hatte). Ursache: `nextCall()` rief den **bestehenden** Aufraeumer nicht, und der gemerkte Ausgang wurde nirgends genullt. **Ein Aufraeumer auf zwei von drei Wegen ist schlimmer als keiner** — er erzeugt die Ueberzeugung, das Problem sei geloest. Gefixt durch **Aufruf statt Nachbau** + Nullung in **derselben** Funktion + Wegfall des Rueckfalls (**fehlt der Ausgang, wird gar keiner gezeigt**). ROT verbatim ueber `triage.sh`, GRUEN 3/3. ⚠ **Zwei Anker-Fehlmessungen dokumentiert, kein Code gebogen:** `postcall/trend`-Sweep zaehlt eine stale `.pyc` (mit `--include` → 0); `grep -c "^def test_" == 2` ist durch Task 2b auf 3 gehoben (Ersatz: die zwei Task-2-Tests namentlich). 🔎 **Funde, nicht mitgefixt:** `_calc_coaching_score` hat nur Test-Aufrufer (→ ERZEUGER-Folgephase) · Redeanteil-Kachel im PiP bleibt (→ 4.0.2) · `state.lastOutcome` hat jetzt **keinen Leser** mehr (→ streichbar). SUMMARY: `08.23.2.METRIK-1-08-SUMMARY.md` | SPEC-R9 | ⛔ **Ausrollen 4 OFFEN** — `deploy.sh production` + Latenz-Abnahme beim Auflegen + Schritt 3b = **Halt-Punkt Andre** |
...
3. **NEU 2026-08-15, beim Ausfuehren von Plan 06 gefunden — die ZEHNTE:** `templates/session_detail.html` Sektion 13, **LIVE**-Empty-State: „… werden ab Phase 4.19 verfuegbar. **Aktuell siehst du den Gesamt-Score und seine 4 Komponenten in Sektion 2.**" Der Satz nannte die geloeschte Note im Live-Zweig, war damit **unwahr** (Punkt 23) **und** machte den Abwesenheits-Anker von Plan 06 Task 1 **per Bauart unerfuellbar**. **Ersatzlos gestrichen** (versorgt in Plan 06). ⚠ Auch dieser Fund traegt weder Funktionsnamen noch Zahl — er war **nur ueber den gerenderten Bildschirm** zu finden, nicht ueber einen Namens-grep.
