...
def api_register():
    ...
    # Plan aus Teamgröße ableiten (alle Flat-Rate Pläne haben max_users=1, Starter ist Standard)
    ...
    try:
        ...
        tok    = secrets.token_urlsafe(32)
        ablauf = now + timedelta(hours=MAX_SESSION_HOURS)
        db_sess = DbSession(user_id=user.id, token=tok, ablauf_am=ablauf)
        db.add(db_sess)
        db.commit()
